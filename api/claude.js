...
// · vérification de session Supabase + crédits IA côté serveur (Premium/Premium+).
//
...
// mais ne sont ni globaux ni persistants (réinitialisés au démarrage à froid). Les CRÉDITS,
// eux, sont désormais comptés dans Supabase (fiable, persistant, partagé entre instances).
...
// Palier -> modèle et plafond de crédits mensuels. Décidés SERVEUR-CÔTÉ à
// partir du profil Supabase vérifié — jamais à partir du modèle demandé par
// le client (qui pourrait mentir).
// Un seul niveau d'accès complet : "member". La valeur "premium_plus" reste
// acceptée pour les comptes activés avant le renommage, afin de ne pas leur
// couper l'accès du jour au lendemain.
const TIER_MODELS = { member: 'claude-opus-4-8', premium_plus: 'claude-opus-4-8' };
const TIER_CAPS   = { member: 150, premium_plus: 150 };
...
// Date ISO (YYYY-MM-DD) dans un mois — prochaine échéance de renouvellement des crédits
...
export default async function handler(req, res) {
  ...
  try {
    const { model: requestedModel, messages, system, max_tokens, access_token } = req.body || {};

    // ── Détermination du modèle ET vérification des crédits, CÔTÉ SERVEUR ──
    // Le modèle n'est JAMAIS pris tel quel depuis le client : soit il est
    // dérivé du palier vérifié auprès de Supabase (connecté), soit il est
    // figé à Sonnet pour un visiteur Free (non connecté, quota client-side
    // inchangé — enjeu faible : 1 question/jour).
    let finalModel;
    let verifiedUserId = null;
    let creditsUsedAfter = null;
    let creditsCap = null;
    ...
    if (access_token) {
      ...
      const { data: profile, error: profileErr } = await supabaseAdmin
        .from('profiles')
        .select('tier, credits_used, credits_reset_at, access_expires_at')
        .eq('id', verifiedUserId)
        .single();
      if (profileErr || !profile) {
        return res.status(403).json({ error: 'PROFILE_INVALID' });
      }

      // Accès accordé via un code temporaire (voir api/redeem.js) : au-delà de
      // l'échéance, le palier reste peut-être encore écrit tel quel en base
      // (le nettoyage client n'a pas forcément eu lieu), mais le serveur ne
      // doit JAMAIS s'y fier après expiration — c'est le vrai verrou de
      // sécurité, le seul qui compte réellement.
      if (profile.access_expires_at && new Date(profile.access_expires_at) <= new Date()) {
        return res.status(403).json({ error: 'ACCESS_EXPIRED' });
      }

      if (!TIER_CAPS[profile.tier]) {
        // Palier 'free' (ou valeur inattendue) : un compte peut très bien être
        // connecté SANS avoir activé de code. On le traite exactement comme un
        // visiteur anonyme — modèle Sonnet figé, aucun crédit décompté, quota
        // quotidien géré côté client (enjeu faible : 1 question/jour).
        finalModel = 'claude-sonnet-4-6';
      } else {
        creditsCap = TIER_CAPS[profile.tier];
        let creditsUsed = profile.credits_used;

      // Renouvellement mensuel si l'échéance est passée
      if (new Date(profile.credits_reset_at) <= new Date()) {
        creditsUsed = 0;
        await supabaseAdmin.from('profiles')
          .update({ credits_used: 0, credits_reset_at: nextMonthISODate() })
          .eq('id', verifiedUserId);
      }

      if (creditsUsed >= creditsCap) {
        return res.status(429).json({ error: 'QUOTA_EXCEEDED', credits_used: creditsUsed, credits_cap: creditsCap });
      }

        finalModel = TIER_MODELS[profile.tier];
        creditsUsedAfter = creditsUsed + 1; // écrit seulement APRÈS le succès de l'appel Anthropic
      }
    } else {
      // Pas de session : palier Free, comportement historique (client-side, hors serveur)
    ...
    // JSON.stringify couvre les deux formats de "system" : chaine simple (avant)
    // ou tableau avec cache_control (prompt caching) -- la garde s'applique aux deux.
    if (JSON.stringify(system ?? '').length > MAX_BODY_CHARS) {
    ...
    // Crédit décompté seulement maintenant : un appel Anthropic en échec ne
    // consomme pas de crédit.
    // creditsUsedAfter reste null pour un compte au palier 'free' : rien à
    // décompter dans ce cas, et surtout rien à écrire en base (sans cette
    // condition, on écrasait le compteur avec une valeur vide).
    if (verifiedUserId && supabaseAdmin && creditsUsedAfter !== null) {
      await supabaseAdmin.from('profiles').update({ credits_used: creditsUsedAfter }).eq('id', verifiedUserId);
      data._credits = { used: creditsUsedAfter, cap: creditsCap };
    }
