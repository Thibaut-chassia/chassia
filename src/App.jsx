...
const UIX_I18N = {
  fr: {
    syncSignIn:"Se connecter avec Google", syncSignOut:"Se déconnecter", syncExplain:"Connecte-toi pour synchroniser tes données entre appareils.", syncConnectedPrefix:"Connecté :", tierUpdateError:"Échec de la mise à jour de ton abonnement. Réessaie dans un instant.",
  ...
  nl: {
    syncSignIn:"Inloggen met Google", syncSignOut:"Afmelden", syncExplain:"Log in om je gegevens tussen toestellen te synchroniseren.", syncConnectedPrefix:"Verbonden:", tierUpdateError:"Bijwerken van je abonnement is mislukt. Probeer het straks opnieuw.",
  ...
  de: {
    syncSignIn:"Mit Google anmelden", syncSignOut:"Abmelden", syncExplain:"Melde dich an, um deine Daten zwischen Geräten zu synchronisieren.", syncConnectedPrefix:"Verbunden:", tierUpdateError:"Aktualisierung deines Abos fehlgeschlagen. Versuch es gleich noch einmal.",
  ...
  en: {
    syncSignIn:"Sign in with Google", syncSignOut:"Sign out", syncExplain:"Sign in to sync your data across devices.", syncConnectedPrefix:"Connected:", tierUpdateError:"Failed to update your subscription. Try again shortly.",
...
function SyncBanner({ session, onSignIn, onSignOut, texts }) {
  if (session) {
    return (
...
export default function ChassIA() {
  ...
  useEffect(() => {
    if (!session) { setProfileTier(null); return; }
    let cancelled = false;
    (async () => {
      ...
      const { data, error } = await supabase.from("profiles").select("tier").eq("id", session.user.id).single();
          ...
          {tab === "carnet" && (
            <div className="fade-in">
              {!isPremium ? (
                ...
                return (
                  <div>
                    {isPremium && <SyncBanner session={session} onSignIn={signInWithGoogle} onSignOut={signOutSupabase} texts={UIX} />}
                    {/* Header */}
