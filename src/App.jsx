...
export default function ChassIA() {
  ...
  const deleteDoc = (id) => {
    const removedType = coffreData[id]?.type || "";
    const updated = { ...coffreData };
    delete updated[id];
    if (saveCoffre(updated)) {
      setCoffreData(updated);
      if (session && isPremiumPlus) {
        const ext = removedType.split("/")[1] || "bin";
        supabase.storage.from("coffre-documents").remove([`${session.user.id}/${id}.${ext}`]);
        supabase.from("coffre_documents").delete().eq("user_id", session.user.id).eq("doc_type", id);
      }
    } else {
      setCoffreError(UIX.coffreErrStorage);
    }
                              ...
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  <div style={{ display:"flex", gap:4 }}>
                                    <button onClick={() => { const u=carnetEntries.filter(e=>e.id!==entry.id); setCarnetEntries(u); saveCarnetWithBackup(u); setConfirmDelete(null); if (session && isPremiumPlus) { supabase.from("carnet_entries").delete().eq("id", entry.id); if ((entry.photos||[]).length > 0) supabase.storage.from("carnet-photos").remove((entry.photos||[]).map((_, i) => `${session.user.id}/${entry.id}/${i}.jpg`)); } }} style={{ background:COLORS.danger, border:"none", borderRadius:5, padding:"4px 10px", color:"white", fontSize:10, cursor:"pointer", fontWeight:700 }}>{CN.confirm}</button>
