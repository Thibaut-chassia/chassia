...
export default function ChassIA() {
  ...
  const [coffreViewer, setCoffreViewer] = useState(null); // { id, label, data, type }
  const [confirmDeleteDoc, setConfirmDeleteDoc] = useState(null);

  const uploadDoc = (id, file) => {
    if (!file) return;
    if (!["image/jpeg","image/png","image/webp","application/pdf"].includes(file.type)) {
      alert("Format non supporté — utilisez une photo (JPG, PNG) ou un PDF."); return;
    }
    if (file.size > 5 * 1024 * 1024) { alert("Fichier trop volumineux — maximum 5 MB."); return; }
  ...
  // Couche 1 : sauvegarde silencieuse localStorage (appelée à chaque modif carnet)
  ...
  // Couche 2 : export fichier JSON (auto premium ou manuel)
  const exportData = () => {
    const data = {
      version: 1,
      exportDate: new Date().toISOString(),
      carnet: carnetEntries,
      lang: localStorage.getItem("chassia_lang") || null,
    };
    const json = JSON.stringify(data, null, 2);
    const blob = new Blob([json], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    // iOS Safari bloque les téléchargements auto — on ouvre dans un nouvel onglet
    // L'utilisateur peut alors "Partager → Enregistrer dans Fichiers"
    const win = window.open(url, "_blank");
    if (!win) {
      // Si popup bloqué, fallback : lien dans le DOM
      const a = document.createElement("a");
      a.href = url;
      a.download = `chassia-backup-${new Date().toISOString().slice(0,10)}.json`;
  ...
  // Couche 3 : restauration depuis fichier JSON
  const importData = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!data.carnet || !Array.isArray(data.carnet)) {
          alert("Fichier invalide — format ChassIA non reconnu."); return;
        }
        saveCarnet(data.carnet);
        setCarnetEntries(data.carnet);
        if (data.lang) localStorage.setItem("chassia_lang", data.lang);
        setLastBackup();
        setLastBackupDate(new Date().toISOString());
        const d = data.exportDate ? new Date(data.exportDate).toLocaleDateString("fr-BE") : "?";
        alert(`✓ ${data.carnet.length} prise${data.carnet.length!==1?"s":""} restaurée${data.carnet.length!==1?"s":""}. Sauvegarde du ${d}.`);
      } catch { alert("Erreur de lecture — vérifiez que c'est bien un fichier ChassIA."); }
    };
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", padding: "20px 24px 0" }}>
                ...
                return (
                  <div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ background: warn ? "#1a1a00" : "#0d1a0d", border:`1px solid ${warn ? "#b8860b" : COLORS.accent}40`, borderRadius:8, padding:"10px 14px", marginBottom:16 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom: isPremium ? 8 : 0 }}>
                            <span style={{ fontSize:14 }}>{warn ? "⚠️" : "✅"}</span>
                            <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color: warn ? "#b8860b" : COLORS.accent, flex:1 }}>
                              {days === null
                                ? "Aucune sauvegarde — enregistrez une prise pour déclencher la sauvegarde."
                                : days === 0 ? "Sauvegarde effectuée aujourd'hui."
                                : `Dernière sauvegarde il y a ${days} jour${days>1?"s":""}${days>=7?" — pensez à sauvegarder !":"."}` }
                            </span>
                          </div>
                          <div style={{ display:"flex", gap:6 }}>
                            <button onClick={exportData}
                              style={{ flex:1, background:"transparent", color:COLORS.accent, border:`1px solid ${COLORS.accent}40`, borderRadius:5, padding:"5px 10px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, cursor:"pointer" }}>
                              ⬇ Exporter
                            </button>
                            <label style={{ flex:1, background:"transparent", color:COLORS.textMuted, border:`1px solid ${COLORS.border}`, borderRadius:5, padding:"5px 10px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, cursor:"pointer", textAlign:"center" }}>
                              ⬆ Restaurer
                              <input type="file" accept=".json" style={{ display:"none" }} onChange={e => { importData(e.target.files[0]); e.target.value=""; }} />
                            </label>
                          </div>
                          {!isPremium && (
                            <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:COLORS.textDim, marginTop:6 }}>
                              ★ Premium : export automatique après chaque prise
                            </div>
                          )}
                        </div>
                      );
                      ...
                      <div style={card}>
                        ...
                        <div style={{ display:"flex", gap:10, marginTop:16 }}>
                          <button onClick={() => {
                            ...
                            setCarnetView("liste");
                            if (isPremium) { setTimeout(exportData, 300); }
                            ...
                            <div key={entry.id} style={{ ...card, padding:"12px 14px" }}>
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  <div style={{ display:"flex", gap:4 }}>
                                    <button onClick={() => { const u=carnetEntries.filter(e=>e.id!==entry.id); setCarnetEntries(u); saveCarnetWithBackup(u); setConfirmDelete(null); }} style={{ background:"#c62828", border:"none", borderRadius:5, padding:"4px 10px", color:"white", fontSize:10, cursor:"pointer", fontWeight:700 }}>Confirmer</button>
          ...
          {tab === "coffre" && (
            <div className="fade-in">
              ...
              <div style={{ background:"#1a2a1a", border:`1px solid ${COLORS.accent}30`, borderRadius:8, padding:"10px 14px", marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:16 }}>🔒</span>
                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.textMuted }}>
                  Vos documents restent sur votre appareil — aucun envoi vers nos serveurs. Max 5 MB par fichier.
                </span>
              </div>

              {/* Restaurer le carnet */}
              <div style={{ background:COLORS.bgInput, border:`1px solid ${COLORS.border}`, borderRadius:8, padding:"10px 14px", marginBottom:20, display:"flex", alignItems:"center", gap:10 }}>
                <span style={{ fontSize:14 }}>📥</span>
                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.textMuted, flex:1 }}>
                  Restaurer votre carnet depuis un fichier de sauvegarde ChassIA (.json)
                </span>
                <label style={{ background:COLORS.accent, color:"#000", border:"none", borderRadius:5, padding:"5px 12px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, fontWeight:700, cursor:"pointer", flexShrink:0 }}>
                  ⬆ Restaurer
                  <input type="file" accept=".json" style={{ display:"none" }} onChange={e => { importData(e.target.files[0]); e.target.value=""; }} />
                </label>
              </div>
                  ...
                  return (
                    <div key={doc.id} style={{ ...card, padding:"14px 16px", display:"flex", alignItems:"center", gap:14 }}>
                      ...
                      <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                        {stored ? (
                          <>
                            <button onClick={() => setCoffreViewer({ ...doc, ...stored })}
                            ...
                            <button onClick={() => { if(confirmDeleteDoc === doc.id) { deleteDoc(doc.id); setConfirmDeleteDoc(null); } else { setConfirmDeleteDoc(doc.id); } }}
                              style={{ background: confirmDeleteDoc === doc.id ? "#c62828" : "transparent", color: confirmDeleteDoc === doc.id ? "white" : "#e57373", border: confirmDeleteDoc === doc.id ? "none" : "1px solid #e5737340", borderRadius:5, padding:"5px 8px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, cursor:"pointer", transition:"all 0.2s" }}>
                              {confirmDeleteDoc === doc.id ? "Confirmer" : "🗑"}
                            </button>
                          </>
                        ) : (
                          <label style={{ background:COLORS.accent, color:"#000", border:"none", borderRadius:5, padding:"5px 12px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, fontWeight:700, cursor:"pointer" }}>
                            + Ajouter
                            <input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" style={{ display:"none" }} onChange={e => { uploadDoc(doc.id, e.target.files[0]); e.target.value=""; }} />
                          </label>
                        )}
              ...
              {coffreViewer && (
                <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.92)", zIndex:1000, display:"flex", flexDirection:"column" }}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"14px 18px", borderBottom:`1px solid ${COLORS.border}` }}
                    ...
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:12, color:COLORS.white, fontWeight:700 }}>
                      {coffreViewer.emoji} {coffreViewer.label}
                    </div>
                    <div style={{ display:"flex", gap:8 }}>
                      <a href={coffreViewer.data} download={coffreViewer.name}
                        style={{ background:COLORS.accent, color:"#000", borderRadius:5, padding:"5px 12px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, fontWeight:700, textDecoration:"none" }}>
                        ⬇ Télécharger
                      </a>
                      <button onClick={() => setCoffreViewer(null)}
                        style={{ background:"transparent", color:COLORS.textMuted, border:`1px solid ${COLORS.border}`, borderRadius:5, padding:"5px 12px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, cursor:"pointer" }}>
                        ✕ Fermer
                      </button>
                    </div>
                  </div>
                  <div style={{ flex:1, overflow:"auto", display:"flex", justifyContent:"center", alignItems:"flex-start", padding:16 }}
                    onClick={e => e.stopPropagation()}>
                    {coffreViewer.type === "application/pdf" ? (
                      <iframe src={coffreViewer.data} style={{ width:"100%", height:"80vh", border:"none", borderRadius:8 }} title={coffreViewer.name} />
                    ) : (
                      <img src={coffreViewer.data} alt={coffreViewer.name} style={{ maxWidth:"100%", maxHeight:"80vh", borderRadius:8, objectFit:"contain" }} />
                    )}
                  </div>
                </div>
              )}
