...
const COFFRE_I18N = {
  fr: {
    intro:"Stockez ici les photos ou PDFs de vos documents importants. Accessibles hors-ligne.",
  ...
  nl: {
    intro:"Bewaar hier de foto's of PDF's van je belangrijke documenten. Offline beschikbaar.",
  ...
  de: {
    intro:"Speichern Sie hier Fotos oder PDFs Ihrer wichtigen Dokumente. Offline verfügbar.",
  ...
  en: {
    intro:"Store photos or PDFs of your important documents here. Available offline.",
...
const UIX_I18N = {
  fr: {
    syncSignIn:"Se connecter avec Google", syncSignOut:"Se déconnecter", syncExplain:"Connecte-toi pour synchroniser tes données entre appareils.", syncConnectedPrefix:"Connecté :", tierUpdateError:"Échec de la mise à jour de ton abonnement. Réessaie dans un instant.", creditsUnit:"crédits", storageFullCloudOk:"Stockage plein sur cet appareil — envoyé en ligne, disponible sur tes autres appareils. Libère de la place pour le voir aussi ici.",
  ...
  nl: {
    syncSignIn:"Inloggen met Google", syncSignOut:"Afmelden", syncExplain:"Log in om je gegevens tussen toestellen te synchroniseren.", syncConnectedPrefix:"Verbonden:", tierUpdateError:"Bijwerken van je abonnement is mislukt. Probeer het straks opnieuw.", creditsUnit:"credits", storageFullCloudOk:"Opslag vol op dit toestel — online verzonden, beschikbaar op je andere toestellen. Maak ruimte vrij om het ook hier te zien.",
  ...
  de: {
    syncSignIn:"Mit Google anmelden", syncSignOut:"Abmelden", syncExplain:"Melde dich an, um deine Daten zwischen Geräten zu synchronisieren.", syncConnectedPrefix:"Verbunden:", tierUpdateError:"Aktualisierung deines Abos fehlgeschlagen. Versuch es gleich noch einmal.", creditsUnit:"Credits", storageFullCloudOk:"Speicher auf diesem Gerät voll — online gespeichert, auf Ihren anderen Geräten verfügbar. Schaffen Sie Platz, um es auch hier zu sehen.",
...
export default function ChassIA() {
  ...
  const uploadDoc = (id, file) => {
    ...
    const store = (data, type) => {
      ...
      const localOk = saveCoffre(updated);
      const cloudEligible = session && isPremiumPlus;
      // Etat local mis a jour meme si la persistance locale a echoue, tant que
      // le cloud prend le relais : le document existe reellement (juste pas
      // mis en cache sur CET appareil), et sera retelecharge au prochain
      // chargement synchronise.
      if (localOk || cloudEligible) setCoffreData(updated);
      if (cloudEligible) uploadCoffreDocToStorage(session.user.id, id, data, type, file.name);
      if (!localOk) setCoffreError(cloudEligible ? UIX.storageFullCloudOk : UIX.coffreErrStorage);
      else setCoffreError("");
    };
  ...
  const deleteDoc = (id) => {
    ...
    if (saveCoffre(updated)) {
      ...
      if (session && isPremiumPlus) {
        // On lit le chemin RÉELLEMENT enregistré plutôt que de le reconstruire
        // depuis le type local : les deux pourraient diverger (remplacement de
        // format, migration ancienne), et on supprimerait alors le mauvais fichier.
        (async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.length > 0) {
        ...
        if (!cancelled && !insErr && inserted) { setCoffreEquip(inserted); saveEquip(inserted); }
      }
    })();
    return () => { cancelled = true; };
  }, [session, isPremiumPlus]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.length > 0) {
        ...
        saveCarnetWithBackup(cloudEntries);
        ...
        if (!cancelled && !insErr && inserted) {
          ...
          setCarnetEntries(withIds); saveCarnetWithBackup(withIds);
                        ...
                        <div style={{ display:"flex", gap:10, marginTop:16 }}>
                          <button onClick={() => {
                            ...
                            const updated = carnetDraft.id
                              ...
                              : [entry, ...carnetEntries];
                            const localOk = saveCarnetWithBackup(updated);
                            const cloudEligible = session && isPremiumPlus;
                            // Etat local mis a jour meme si la persistance locale a echoue, tant
                            // que le cloud prend le relais (meme logique que le Coffre) : sans ca,
                            // un stockage local plein empêchait TOUJOURS la synchronisation, alors
                            // que c'est justement le cas où elle est le plus utile.
                            if (localOk || cloudEligible) {
                            ...
                            if (!localOk) {
                              setCarnetSaveError(cloudEligible ? UIX.storageFullCloudOk : CN.saveErr);
                              if (!cloudEligible) return; // rien n'a été sauvegardé nulle part : reste sur le formulaire
                            } else {
                              setCarnetSaveError("");
                            }
                              ...
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  <div style={{ display:"flex", gap:4 }}>
                                    <button onClick={() => { const u=carnetEntries.filter(e=>e.id!==entry.id); setCarnetEntries(u); saveCarnetWithBackup(u); setConfirmDelete(null); if (session && isPremiumPlus) { supabase.from("carnet_entries").delete().eq("id", entry.id); removeCarnetPhotoFolder(session.user.id, entry.id); } }} style={{ background:COLORS.danger, border:"none", borderRadius:5, padding:"4px 10px", color:"white", fontSize:10, cursor:"pointer", fontWeight:700 }}>{CN.confirm}</button>
                        ...
                        <div key={e.id} style={{ ...card, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                          ...
                          <button onClick={() => {
                              if (confirmDeleteEquip === e.id) {
                                ...
                                if (saveEquip(updated)) {
                                  setCoffreEquip(updated);
                                  if (session && isPremiumPlus) supabase.from("coffre_equipement").delete().eq("id", e.id);
                          ...
                          <div style={{ display:"flex", gap:8 }}>
                            <button onClick={() => {
                                ...
                                if (saveEquip(updated)) {
                                  ...
                                  if (session && isPremiumPlus) supabase.from("coffre_equipement").insert({ id:entry.id, user_id:session.user.id, cat:entry.cat, titre:entry.titre, detail:entry.detail });
                                }
                                else setEquipError(UIX.coffreErrStorage);
                              }}
