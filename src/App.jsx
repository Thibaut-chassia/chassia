...
// ─── SUPABASE (sync cloud membres) ──────────────────────────────────────────
const supabase = createClient(
  "https://tcnvqpwdtunmxrngisyn.supabase.co",
  "sb_publishable_GStfDjxnTklKkdHm59zF0g_J1ov6ZM2"
);
...
const LANGS = {
  fr: {
    ...
    premiumBtn:"Accès membre", premiumActive:"★ Membre", premiumPlusActive:"★ Membre",
    limitTitle:"Limite quotidienne atteinte",
    limitText:"Tu as utilisé ta question gratuite du jour. Connecte-toi avec ton code d'accès pour 150 crédits IA par mois.",
    assistPH:"Pose ta question sur la chasse en Belgique… (Entrée pour envoyer)",
    assistLimitPH:"Limite quotidienne atteinte — connecte-toi",
    freeNote:"Visiteur : 1 question / jour · 150 crédits IA / mois pour les membres",
  ...
  nl: {
    ...
    premiumBtn:"Ledentoegang", premiumActive:"★ Lid", premiumPlusActive:"★ Lid",
    limitTitle:"Dagelijkse limiet bereikt",
    limitText:"Je hebt je gratis vraag van vandaag gebruikt. Log in met je toegangscode voor 150 AI-credits per maand.",
    assistPH:"Stel je vraag over de jacht in België… (Enter om te verzenden)",
    assistLimitPH:"Daglimiet bereikt — log in",
    freeNote:"Bezoeker: 1 vraag/dag · 150 AI-credits/maand voor leden",
  ...
  de: {
    ...
    premiumBtn:"Mitgliederzugang", premiumActive:"★ Mitglied", premiumPlusActive:"★ Mitglied",
    limitTitle:"Tageslimit erreicht",
    limitText:"Sie haben Ihre kostenlose Frage für heute verbraucht. Melden Sie sich mit Ihrem Zugangscode an für 150 KI-Credits pro Monat.",
    assistPH:"Stellen Sie Ihre Frage zur Jagd in Belgien… (Enter zum Senden)",
    assistLimitPH:"Tageslimit erreicht — anmelden",
    freeNote:"Besucher: 1 Frage/Tag · 150 KI-Credits/Monat für Mitglieder",
  ...
  en: {
    ...
    premiumBtn:"Member access", premiumActive:"★ Member", premiumPlusActive:"★ Member",
    limitTitle:"Daily limit reached",
    limitText:"You have used your free question for today. Sign in with your access code for 150 AI credits per month.",
    assistPH:"Ask your question about hunting in Belgium… (Enter to send)",
    assistLimitPH:"Daily limit reached — sign in",
    freeNote:"Visitor: 1 question/day · 150 AI credits/month for members",
...
// ─── API CALL ────────────────────────────────────────────────────────────────
async function askClaude(prompt, systemPrompt, history = [], premium = false, timeoutMsg = "", offlineMsg = "", accessToken = null, onCredits = null) {
  ...
  try {
    res = await fetch("/api/claude", {
      ...
      body: JSON.stringify({
        ...
        messages,
        // Jeton de session Supabase : permet au serveur de vérifier le vrai palier
        // et de décompter les crédits — absent pour un visiteur Free (non connecté).
  ...
  if (data && data.content && data.content[0] && data.content[0].text) {
    if (data._credits && onCredits) onCredits(data._credits.used, data._credits.cap);
...
const CARNET_I18N = {
  fr: {
    ...
    gateDesc:"Notez chaque prise, suivez vos statistiques saison par saison. Réservé aux membres.",
    title:"📖 Carnet de chasse", season:"Saison", take:"prise", takes:"prises", newTake:"+ Nouvelle prise",
    backupWarn:"Vos données sont sauvegardées sur cet appareil uniquement. Si vous changez d'appareil ou videz votre navigateur, elles seront perdues. Synchronisation disponible pour les membres.", backupSynced:"Tes données sont synchronisées avec ton compte — accessibles sur tous tes appareils connectés.",
  ...
  nl: {
    ...
    gateDesc:"Noteer elk stuk, volg je statistieken seizoen per seizoen. Voorbehouden aan leden.",
    title:"📖 Jachtlogboek", season:"Seizoen", take:"stuk", takes:"stukken", newTake:"+ Nieuw stuk",
    backupWarn:"Je gegevens worden enkel op dit toestel opgeslagen. Als je van toestel verandert of je browser wist, gaan ze verloren. Synchronisatie beschikbaar voor leden.", backupSynced:"Je gegevens zijn gesynchroniseerd met je account — toegankelijk op al je verbonden toestellen.",
  ...
  de: {
    ...
    gateDesc:"Erfassen Sie jede Erlegung, verfolgen Sie Ihre Statistiken Saison für Saison. Mitgliedern vorbehalten.",
    title:"📖 Jagdbuch", season:"Saison", take:"Stück", takes:"Stück", newTake:"+ Neue Erlegung",
    backupWarn:"Ihre Daten werden nur auf diesem Gerät gespeichert. Wenn Sie das Gerät wechseln oder den Browser leeren, gehen sie verloren. Synchronisierung für Mitglieder verfügbar.", backupSynced:"Ihre Daten sind mit Ihrem Konto synchronisiert — auf allen verbundenen Geräten verfügbar.",
  ...
  en: {
    ...
    gateDesc:"Record every harvest, track your statistics season by season. Members only.",
    title:"📖 Hunting logbook", season:"Season", take:"entry", takes:"entries", newTake:"+ New entry",
    backupWarn:"Your data is saved on this device only. If you change device or clear your browser, it will be lost. Sync available for members.", backupSynced:"Your data is synced with your account — accessible on all your connected devices.",
...
const COFFRE_I18N = {
  fr: {
    intro:"Stockez ici les photos ou PDFs de vos documents importants. Accessibles hors-ligne.", introSynced:"Stockez ici les photos ou PDFs de vos documents importants.",
    banner:"Vos documents sont sauvegardés sur cet appareil uniquement. Si vous changez d'appareil ou videz votre navigateur, ils seront perdus. Synchronisation disponible pour les membres.", bannerSynced:"Tes documents sont synchronisés avec ton compte — accessibles sur tous tes appareils connectés.",
    view:"👁 Voir", replace:"Remplacer", confirm:"Confirmer", add:"+ Ajouter", close:"✕ Fermer", gateTitle:"Coffre-fort numérique", gateDesc:"Stockez vos documents de chasse en lieu sûr : permis, assurance, certificats. Réservé aux membres.",
  ...
  nl: {
    intro:"Bewaar hier de foto's of PDF's van je belangrijke documenten. Offline beschikbaar.", introSynced:"Bewaar hier de foto's of PDF's van je belangrijke documenten.",
    banner:"Je documenten worden enkel op dit toestel opgeslagen. Als je van toestel verandert of je browser wist, gaan ze verloren. Synchronisatie beschikbaar voor leden.", bannerSynced:"Je documenten zijn gesynchroniseerd met je account — toegankelijk op al je verbonden toestellen.",
    view:"👁 Bekijken", replace:"Vervangen", confirm:"Bevestigen", add:"+ Toevoegen", close:"✕ Sluiten", gateTitle:"Digitale kluis", gateDesc:"Bewaar je jachtdocumenten veilig: jachtverlof, verzekering, certificaten. Voorbehouden aan leden.",
  ...
  de: {
    intro:"Speichern Sie hier Fotos oder PDFs Ihrer wichtigen Dokumente. Offline verfügbar.", introSynced:"Speichern Sie hier Fotos oder PDFs Ihrer wichtigen Dokumente.",
    banner:"Ihre Dokumente werden nur auf diesem Gerät gespeichert. Wenn Sie das Gerät wechseln oder den Browser leeren, gehen sie verloren. Synchronisierung für Mitglieder verfügbar.", bannerSynced:"Ihre Dokumente sind mit Ihrem Konto synchronisiert — auf allen verbundenen Geräten verfügbar.",
    view:"👁 Ansehen", replace:"Ersetzen", confirm:"Bestätigen", add:"+ Hinzufügen", close:"✕ Schließen", gateTitle:"Digitaler Tresor", gateDesc:"Bewahren Sie Ihre Jagddokumente sicher auf: Jagdschein, Versicherung, Bescheinigungen. Mitgliedern vorbehalten.",
  ...
  en: {
    intro:"Store photos or PDFs of your important documents here. Available offline.", introSynced:"Store photos or PDFs of your important documents here.",
    banner:"Your documents are saved on this device only. If you change device or clear your browser, they will be lost. Sync available for members.", bannerSynced:"Your documents are synced with your account — accessible on all your connected devices.",
    view:"👁 View", replace:"Replace", confirm:"Confirm", add:"+ Add", close:"✕ Close", gateTitle:"Digital safe", gateDesc:"Store your hunting documents securely: licence, insurance, certificates. Members only.",
...
const UIX_I18N = {
  fr: {
    syncSignIn:"Se connecter avec Google", syncSignOut:"Se déconnecter", syncExplain:"Connecte-toi pour synchroniser tes données entre appareils.", syncConnectedPrefix:"Connecté :", creditsUnit:"crédits", redeemPlaceholder:"Ton code", redeemBtn:"Activer", redeemSuccess:"Code activé ! Ton palier est mis à jour.", redeemErrNotFound:"Code introuvable.", redeemErrUsed:"Ce code a déjà été utilisé.", redeemErrSession:"Connecte-toi avec Google pour activer ton code.", redeemErrGeneric:"Une erreur est survenue, réessaie dans un instant.",
    ...
    addGame:"— Ajouter un gibier (jusqu'à 5) —", clearAll:"Effacer tout", munOptional:"Munition (optionnel)", upTo5:"jusqu'à 5", munition:"Munition", calRec:"Calibres recommandés", calibreStar:"Calibre *", calibreLabel:"Calibre", distanceM:"Distance (m)", wetlandMandatory:"Obligatoire zones humides", atDist:"à", modeDescBattue:"Ogives à expansion rapide — choc et arrêt immédiats pour la battue", modeDescAffut:"Ogives longue portée — expansion progressive et haute rétention de masse", modeDescPoly:"Ogives polyvalentes — efficaces sur toutes distances", moreChoicesBtn:"🤖 Choix supplémentaires & conseil IA approfondi", deepAdviceBtn:"🤖 Conseil IA approfondi", federal:"Fédéral", notApplicable:"— Non applicable —", modeCarabine:"🔫 Carabine", modeFusil:"🦆 Fusil", modeComparer:"📊 Comparer", opusAnalysis:"Assistant IA sur Claude Opus — analyse approfondie", sonnetAnalysis:"Assistant IA sur Claude Sonnet", upgradeNudge:"Réservé aux membres — connecte-toi avec ton code d'accès →", creditsLabel:"crédits IA / mois (toutes fonctions confondues)", quotaMsgPrefix:"⚠️ Limite mensuelle de ", quotaMsgSuffix:" crédits IA atteinte (toutes fonctions IA confondues : Assistant, Conseil calibre, Conseil munition). Elle se renouvelle automatiquement chaque mois.", premiumPlusBtn:"Accès membre", examplesTitle:"Quelques exemples de questions :", suggestion:"Suggestion", coffreErrFormat:"Format non supporté — utilisez une photo (JPG, PNG) ou un PDF.", coffreErrSize:"Fichier trop volumineux — maximum 10 MB.", coffreErrStorage:"Espace de stockage plein sur cet appareil. Supprimez un document existant avant d'en ajouter un nouveau.", leadFreeShort:"🌿 Sans plomb", leadShort:"Plomb", compareIntro:"Compare la trajectoire de plusieurs munitions du même calibre.", compareMax3:"Jusqu'à 3 munitions.", compareMax4:"Jusqu'à 4 munitions.", compareFree:"Visiteur : 2 munitions (4 pour les membres).", chartLegend:"Valeurs en cm vs ligne de visée", chartDelta:" · Δ = écart entre les deux", fusilRec:"🔫 Fusils recommandés", aiDeepBtn:"🤖 Analyse IA approfondie — conseil personnalisé", clearConv:"✕ Effacer la conversation", aiTimeout:"La requête a expiré (30 s). Vérifie ta connexion et réessaie.", aiOffline:"Connexion impossible. Vérifie ta connexion internet.", coffreErrRead:"Impossible de lire ce fichier. Il est peut-être endommagé — réessaie avec un autre.", selected:"sélectionné", droCompLabel:"Distance de réglage (DRO)", compSelectOne:"Sélectionne au moins une munition pour voir sa trajectoire.",
  ...
  nl: {
    syncSignIn:"Inloggen met Google", syncSignOut:"Afmelden", syncExplain:"Log in om je gegevens tussen toestellen te synchroniseren.", syncConnectedPrefix:"Verbonden:", creditsUnit:"credits", redeemPlaceholder:"Je code", redeemBtn:"Activeren", redeemSuccess:"Code geactiveerd! Je niveau is bijgewerkt.", redeemErrNotFound:"Code niet gevonden.", redeemErrUsed:"Deze code is al gebruikt.", redeemErrSession:"Log in met Google om je code te activeren.", redeemErrGeneric:"Er is een fout opgetreden, probeer het straks opnieuw.",
  ...
  de: {
    syncSignIn:"Mit Google anmelden", syncSignOut:"Abmelden", syncExplain:"Melde dich an, um deine Daten zwischen Geräten zu synchronisieren.", syncConnectedPrefix:"Verbunden:", creditsUnit:"Credits", redeemPlaceholder:"Dein Code", redeemBtn:"Aktivieren", redeemSuccess:"Code aktiviert! Dein Level wurde aktualisiert.", redeemErrNotFound:"Code nicht gefunden.", redeemErrUsed:"Dieser Code wurde bereits verwendet.", redeemErrSession:"Melde dich mit Google an, um deinen Code zu aktivieren.", redeemErrGeneric:"Ein Fehler ist aufgetreten, versuch es gleich noch einmal.",
  ...
  en: {
    syncSignIn:"Sign in with Google", syncSignOut:"Sign out", syncExplain:"Sign in to sync your data across devices.", syncConnectedPrefix:"Connected:", creditsUnit:"credits", redeemPlaceholder:"Your code", redeemBtn:"Activate", redeemSuccess:"Code activated! Your tier has been updated.", redeemErrNotFound:"Code not found.", redeemErrUsed:"This code has already been used.", redeemErrSession:"Sign in with Google to activate your code.", redeemErrGeneric:"Something went wrong, try again shortly.",
    ...
    addGame:"— Add game (up to 5) —", clearAll:"Clear all", munOptional:"Ammunition (optional)", upTo5:"up to 5", munition:"Ammunition", calRec:"Recommended calibers", calibreStar:"Caliber *", calibreLabel:"Caliber", distanceM:"Distance (m)", wetlandMandatory:"Mandatory in wetlands", atDist:"at", modeDescBattue:"Rapid-expansion bullets — immediate shock and stop for driven hunts", modeDescAffut:"Long-range bullets — progressive expansion and high weight retention", modeDescPoly:"Versatile bullets — effective at all ranges", moreChoicesBtn:"🤖 Additional choices & in-depth AI advice", deepAdviceBtn:"🤖 In-depth AI advice", federal:"Federal", notApplicable:"— Not applicable —", modeCarabine:"🔫 Rifle", modeFusil:"🦆 Shotgun", modeComparer:"📊 Compare", opusAnalysis:"AI Assistant on Claude Opus — in-depth analysis", sonnetAnalysis:"AI Assistant on Claude Sonnet", upgradeNudge:"Members only — sign in with your access code →", creditsLabel:"AI credits / month (all features combined)", quotaMsgPrefix:"⚠️ Monthly limit of ", quotaMsgSuffix:" AI credits reached (all AI features combined: Assistant, Caliber advice, Ammunition advice). Automatically renews every month.", premiumPlusBtn:"Member access", examplesTitle:"Some example questions:", suggestion:"Suggestion", coffreErrFormat:"Unsupported format — use a photo (JPG, PNG) or a PDF.", coffreErrSize:"File too large — maximum 10 MB.", coffreErrStorage:"Storage full on this device. Delete an existing document before adding a new one.", leadFreeShort:"🌿 Lead-free", leadShort:"Lead", compareIntro:"Compare the trajectory of several ammunitions of the same caliber.", compareMax3:"Up to 3 ammunitions.", compareMax4:"Up to 4 ammunitions.", compareFree:"Visitor: 2 ammunitions (4 for members).", chartLegend:"Values in cm vs line of sight", chartDelta:" · Δ = difference between the two", fusilRec:"🔫 Recommended shotguns", aiDeepBtn:"🤖 In-depth AI analysis — personalized advice", clearConv:"✕ Clear conversation", aiTimeout:"The request timed out (30 s). Check your connection and try again.", aiOffline:"Connection failed. Check your internet connection.", coffreErrRead:"This file could not be read. It may be damaged — try another one.", selected:"selected", droCompLabel:"Zero distance (DRO)", compSelectOne:"Select at least one ammunition to see its trajectory.",
...
const TIER_MODAL_I18N = {
  fr: {
    title: "Accès membre",
    subtitle: "ChassIA est réservé aux membres. Connecte-toi avec ton compte Google pour retrouver ton accès, ou saisis ton code d'accès personnel.",
    connectBtn: "Se connecter avec Google",
    visitorName: "Visiteur",
    visitorFeatures: ["Réglage de carabine et balistique", "522 munitions sur 44 calibres", "Comparateur jusqu'à 2 munitions", "Fiches gibier", "1 question à l'assistant IA par jour"],
    memberName: "Membre",
    memberFeatures: ["Tout ce dont dispose un visiteur", "Carnet de chasse — photos comprises, jusqu'à 3 par prise", "Coffre-fort — documents et fiche équipement", "Comparateur jusqu'à 4 munitions", "Rapport de saison en PDF", "150 crédits IA par mois", "Réponses sur Claude Opus — analyses plus approfondies", "Synchronisation entre tous tes appareils"],
    currentBadge: "Ton accès",
    close: "Fermer",
  },
  nl: {
    title: "Ledentoegang",
    subtitle: "ChassIA is voorbehouden aan leden. Log in met je Google-account om je toegang terug te vinden, of voer je persoonlijke toegangscode in.",
    connectBtn: "Inloggen met Google",
    visitorName: "Bezoeker",
    visitorFeatures: ["Karabijnafstelling en ballistiek", "522 munities voor 44 kalibers", "Vergelijker tot 2 munities", "Wildfiches", "1 vraag per dag aan de AI-assistent"],
    memberName: "Lid",
    memberFeatures: ["Alles wat een bezoeker heeft", "Jachtlogboek — inclusief foto's, tot 3 per vangst", "Kluis — documenten en uitrustingsfiche", "Vergelijker tot 4 munities", "Seizoensrapport in PDF", "150 AI-credits per maand", "Antwoorden via Claude Opus — diepgaandere analyses", "Synchronisatie tussen al je toestellen"],
    currentBadge: "Jouw toegang",
    close: "Sluiten",
  },
  de: {
    title: "Mitgliederzugang",
    subtitle: "ChassIA ist Mitgliedern vorbehalten. Melden Sie sich mit Ihrem Google-Konto an, um Ihren Zugang wiederzufinden, oder geben Sie Ihren persönlichen Zugangscode ein.",
    connectBtn: "Mit Google anmelden",
    visitorName: "Besucher",
    visitorFeatures: ["Büchseneinstellung und Ballistik", "522 Munitionen für 44 Kaliber", "Vergleich von bis zu 2 Munitionen", "Wildarten-Steckbriefe", "1 Frage pro Tag an den KI-Assistenten"],
    memberName: "Mitglied",
    memberFeatures: ["Alles, was ein Besucher hat", "Jagdbuch — mit Fotos, bis zu 3 pro Abschuss", "Tresor — Dokumente und Ausrüstungsliste", "Vergleich von bis zu 4 Munitionen", "Saisonbericht als PDF", "150 KI-Credits pro Monat", "Antworten über Claude Opus — tiefergehende Analysen", "Synchronisierung zwischen all Ihren Geräten"],
    currentBadge: "Ihr Zugang",
    close: "Schließen",
  },
  en: {
    title: "Member access",
    subtitle: "ChassIA is reserved for members. Sign in with your Google account to restore your access, or enter your personal access code.",
    connectBtn: "Sign in with Google",
    visitorName: "Visitor",
    visitorFeatures: ["Rifle zeroing and ballistics", "522 ammunitions across 44 calibres", "Compare up to 2 ammunitions", "Game species sheets", "1 question per day to the AI assistant"],
    memberName: "Member",
    memberFeatures: ["Everything a visitor gets", "Hunting logbook — photos included, up to 3 per entry", "Safe — documents and equipment list", "Compare up to 4 ammunitions", "Season report as PDF", "150 AI credits per month", "Answers on Claude Opus — deeper analysis", "Sync across all your devices"],
    currentBadge: "Your access",
    close: "Close",
  },
};
...
function SyncBanner({ session, onSignIn, onSignOut, texts, creditsUsed, creditsCap }) {
  if (session) {
    return (
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap", background:"rgba(143,176,32,0.08)", border:`1px solid ${COLORS.accent}40`, borderRadius:8, padding:"10px 14px", marginBottom:16 }}>
        <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:COLORS.accent }}>
          ✓ {texts.syncConnectedPrefix} {session.user.email}
          {creditsCap != null && creditsUsed != null && <> · {creditsUsed}/{creditsCap} {texts.creditsUnit}</>}
        </span>
        <button onClick={onSignOut} style={{ background:"transparent", border:"none", color:COLORS.textMuted, fontFamily:"'IBM Plex Mono',monospace", fontSize:11, cursor:"pointer", textDecoration:"underline" }}>
          {texts.syncSignOut}
        </button>
      </div>
    );
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap", background:COLORS.bgCard, border:`1px solid ${COLORS.border}`, borderRadius:8, padding:"10px 14px", marginBottom:16 }}>
      <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:COLORS.textMuted }}>
        {texts.syncExplain}
      </span>
      <button onClick={onSignIn} style={{ background:COLORS.accent, color:COLORS.onAccent, border:"none", borderRadius:6, padding:"8px 14px", fontFamily:"'IBM Plex Mono',monospace", fontSize:11, fontWeight:700, cursor:"pointer", whiteSpace:"nowrap" }}>
        {texts.syncSignIn}
      </button>
    </div>
  );
...
export default function ChassIA() {
  ...
  // ── SUPABASE AUTH (sync cloud membres) ──────────────────────────────────
  ...
  const signOutSupabase = () => { supabase.auth.signOut(); };
  // Identifiant stable : l'objet session est remplacé à chaque événement
  // d'authentification (rafraîchissement automatique du jeton, retour de
  // l'app au premier plan...). S'en servir comme dépendance d'effet
  // relançait une resynchronisation complète à ces moments-là, écrasant
  // l'affichage avec l'état du cloud — d'où des entrées qui « disparaissaient »
  // peu après avoir été ajoutées. L'identifiant, lui, ne change qu'à une
  // vraie connexion/déconnexion.
  ...
  // ── Utilitaires Supabase Storage (photos Carnet, documents Coffre) ──────
  // Convertit une data URI base64 (format utilisé partout en local) en Blob
  // uploadable. fetch() gère nativement le schéma "data:", pas besoin de
  // parser le base64 à la main.
  const dataURItoBlob = async (dataURI) => (await fetch(dataURI)).blob();
  // Sens inverse : Blob téléchargé depuis Storage -> data URI, pour rester
  // compatible avec tout le code d'affichage existant (qui attend du base64).
  ...
  // Photos du Carnet (membres, max 3 par prise) — réutilise compressImage du Coffre
  ...
  // Upload d'un lot de photos (data URIs) vers le bucket carnet-photos, sous
  // {user_id}/{entry_id}/{index}.jpg. Retourne les chemins effectivement
  // envoyés (une photo en échec est simplement omise, pas d'erreur bloquante
  // — l'entrée texte est déjà sauvegardée de toute façon).
  // Supprime tous les fichiers du dossier photos d'une entrée. Utilisé à la
  // suppression d'une entrée, et avant chaque réupload (une modification peut
  // réduire le nombre de photos, laissant sinon des fichiers orphelins).
  const removeCarnetPhotoFolder = async (userId, entryId) => {
    try {
      const { data: existing } = await supabase.storage.from("carnet-photos").list(`${userId}/${entryId}`);
      if (existing && existing.length > 0) {
        await supabase.storage.from("carnet-photos").remove(existing.map(f => `${userId}/${entryId}/${f.name}`));
      }
    } catch { /* nettoyage best-effort : ne doit jamais bloquer l'appelant */ }
  };

  const uploadCarnetPhotos = async (userId, entryId, photos) => {
    // Repart d'un dossier propre (voir removeCarnetPhotoFolder ci-dessus)
    await removeCarnetPhotoFolder(userId, entryId);
    const paths = [];
    for (let i = 0; i < photos.length; i++) {
      try {
        const blob = await dataURItoBlob(photos[i]);
        const path = `${userId}/${entryId}/${i}.jpg`;
        const { error } = await supabase.storage.from("carnet-photos").upload(path, blob, { contentType: "image/jpeg", upsert: true });
        if (!error) paths.push(path);
      } catch { /* photo individuelle en échec : ignorée, pas bloquant */ }
    }
    return paths;
  };
  // Sens inverse : chemins de stockage -> data URIs affichables (même format
  // que les photos ajoutées localement, aucun autre code à adapter).
  const downloadCarnetPhotos = async (paths) => {
    const out = [];
    for (const path of paths) {
      try {
        const { data, error } = await supabase.storage.from("carnet-photos").download(path);
        if (!error && data) out.push(await blobToDataURI(data));
      } catch { /* photo individuelle en échec : ignorée */ }
    }
    return out;
  };
  ...
  // Ouvre une photo du Carnet en grand dans un nouvel onglet — même pattern
  // que le bouton "Voir" du Coffre pour les images (cohérence dans l'app).
  const openCarnetPhoto = (src) => {
    const w = window.open();
    w.document.write(`<html><head><title>${CN.photos}</title><style>body{margin:0;background:#000;display:flex;flex-direction:column;align-items:center}button{position:fixed;top:12px;right:12px;background:#8fb020;color:#000;border:none;border-radius:8px;padding:8px 16px;font-size:14px;font-weight:700;cursor:pointer;z-index:9999}img{width:100%;height:auto;display:block;margin-top:0}</style></head><body><button onclick="window.close()">${CF.close}</button><img src="${src}"/></body></html>`);
    w.document.close();
  };

  // Upload d'un document du Coffre vers le bucket coffre-documents, sous
  // {user_id}/{doc_type}.{ext} — un seul fichier actif par type (upsert).
  // Best-effort : un échec n'empêche jamais le document de rester utilisable
  // en local (déjà sauvegardé avant cet appel).
  const uploadCoffreDocToStorage = async (userId, docType, dataURI, mimeType, fileName) => {
    try {
      const blob = await dataURItoBlob(dataURI);
      const ext = mimeType.split("/")[1] || "bin";
      const path = `${userId}/${docType}.${ext}`;
      // Un remplacement peut changer l'extension (ex. PDF remplacé par une
      // image compressée) : l'ancien fichier aurait alors un AUTRE chemin et
      // resterait orphelin à jamais. On supprime donc l'ancien enregistrement
      // s'il pointait ailleurs, avant d'écrire le nouveau.
      const { data: prev } = await supabase.from("coffre_documents")
        .select("file_path").eq("user_id", userId).eq("doc_type", docType).maybeSingle();
      if (prev && prev.file_path && prev.file_path !== path) {
        await supabase.storage.from("coffre-documents").remove([prev.file_path]);
      }
      const { error } = await supabase.storage.from("coffre-documents").upload(path, blob, { contentType: mimeType, upsert: true });
      if (error) return;
      await supabase.from("coffre_documents").upsert(
        { user_id: userId, doc_type: docType, file_path: path, file_name: fileName, mime_type: mimeType },
        { onConflict: "user_id,doc_type" }
      );
    } catch { /* échec best-effort, non bloquant */ }
  };
  ...
  const uploadDoc = (id, file) => {
    ...
    const store = (data, type) => {
      ...
      const cloudEligible = session && isMember;
      const localOk = cloudEligible ? true : saveCoffre(updated);
      if (localOk) setCoffreData(updated);
      if (cloudEligible) uploadCoffreDocToStorage(session.user.id, id, data, type, file.name);
      if (!localOk) setCoffreError(UIX.coffreErrStorage);
      else setCoffreError("");
    };
  ...
  const deleteDoc = async (id) => {
    ...
    const cloudEligible = session && isMember;
    if (cloudEligible) {
      // On ATTEND la confirmation avant de considérer la suppression comme
      // faite — même raison que le Carnet : sans ça, une fermeture rapide de
      // l'app pouvait interrompre la requête, et le document réapparaissait
      // au prochain chargement synchronisé (le cloud, jamais réellement
      // modifié, faisait référence).
      const { data: row } = await supabase.from("coffre_documents")
        .select("file_path").eq("user_id", session.user.id).eq("doc_type", id).maybeSingle();
      if (row && row.file_path) await supabase.storage.from("coffre-documents").remove([row.file_path]);
      await supabase.from("coffre_documents").delete().eq("user_id", session.user.id).eq("doc_type", id);
      setCoffreData(updated);
    } else if (saveCoffre(updated)) {
      setCoffreData(updated);
    } else {
  ...
  const [gibierCMulti, setGibierCMulti] = useState([]); // membres : multi-sélection
  ...
  const [gibierMMulti, setGibierMMulti] = useState([]); // membres : multi-sélection
  ...
  // Palier : "free" (visiteur, Sonnet) | "member" (membre, Opus).
  // "premiumPlus" reste accepté en lecture pour les comptes historiques.
  const tier = "free"; // valeur pour un visiteur non connecté
  // Une fois connecté, le vrai palier vient du profil Supabase — il ne peut
  // désormais être obtenu QUE via un code d'accès valide (voir activateCode).
  const [profileTier, setProfileTier] = useState(null);
  const [creditsUsed, setCreditsUsed] = useState(null); // crédits déjà consommés ce mois-ci (source : Supabase)
  const [accessExpiresAt, setAccessExpiresAt] = useState(null); // échéance d'un accès accordé par code (null = pas d'échéance)
  const [redeemStatus, setRedeemStatus] = useState("idle"); // idle | loading | success | error
  const [redeemMessage, setRedeemMessage] = useState("");
  useEffect(() => {
    if (!session) { setProfileTier(null); setCreditsUsed(null); setAccessExpiresAt(null); return; }
    let cancelled = false;
    (async () => {
      // Un code d'accès était-il en attente (saisi avant d'être connecté) ?
      let pendingCode = null;
      try { pendingCode = localStorage.getItem("chassia_pending_code"); } catch {}
      if (pendingCode) {
        try { localStorage.removeItem("chassia_pending_code"); } catch {}
        try {
          const res = await fetch("/api/redeem", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ code: pendingCode, access_token: session.access_token }),
          });
          const data = await res.json();
          if (res.ok && data.ok) {
            setRedeemStatus("success");
            setRedeemMessage(UIX.redeemSuccess);
          } else {
            const errMap = { CODE_NOT_FOUND: UIX.redeemErrNotFound, CODE_ALREADY_USED: UIX.redeemErrUsed };
            setRedeemStatus("error");
            setRedeemMessage(errMap[data.error] || UIX.redeemErrGeneric);
          }
        } catch { setRedeemStatus("error"); setRedeemMessage(UIX.redeemErrGeneric); }
        // La connexion Google a rechargé l'app : la modale s'est refermée entre
        // temps. On la rouvre pour que le résultat de l'activation soit
        // réellement visible — sinon la personne ne saurait jamais si son code
        // est passé ou non.
        setShowTierModal(true);
      }
      const { data, error } = await supabase.from("profiles").select("tier, credits_used, access_expires_at").eq("id", session.user.id).single();
      if (cancelled || error || !data) return;
      // La base stocke "premium_plus" (convention SQL) ; le code interne utilise
      // "premiumPlus". On normalise ici pour que les deux coïncident.
      setProfileTier(data.tier === "premium_plus" ? "premiumPlus" : data.tier); // "premiumPlus" = comptes activés avant le renommage
      setCreditsUsed(data.credits_used);
      setAccessExpiresAt(data.access_expires_at);
    })();
    return () => { cancelled = true; };
  }, [session]);
  // Un accès accordé par code au-delà de son échéance ne doit plus déverrouiller
  // les fonctions payantes, même si la colonne tier n'a pas encore été
  // "nettoyée" en base — le serveur applique la même règle indépendamment
  // (voir api/claude.js), c'est ce contrôle-là qui compte réellement.
  const isAccessExpired = !!(accessExpiresAt && new Date(accessExpiresAt) <= new Date());
  const effectiveTier = session && !isAccessExpired ? profileTier : tier;
  const [redeemCode, setRedeemCode] = useState("");
  // Le palier accordé est déterminé par le CODE lui-même (côté serveur), pas
  // par le champ dans lequel il a été saisi — les deux champs partagent donc
  // la même logique, seul le contenu du code compte.
  const activateCode = async (rawCode, clearField) => {
    const code = (rawCode || "").trim();
    if (!code) return;
    if (!session) {
      // Pas connecté : on mémorise le code et on lance la connexion — il sera
      // activé automatiquement au retour.
      try { localStorage.setItem("chassia_pending_code", code); } catch {}
      signInWithGoogle();
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.ok) {
        const newTier = data.tier === "premium_plus" ? "premiumPlus" : data.tier;
        setProfileTier(newTier);
        setCreditsUsed(0);
        ...
        const errMap = { CODE_NOT_FOUND: UIX.redeemErrNotFound, CODE_ALREADY_USED: UIX.redeemErrUsed, SESSION_INVALID: UIX.redeemErrSession };
        setRedeemStatus("error");
        setRedeemMessage(errMap[data.error] || UIX.redeemErrGeneric);
      }
    } catch {
      setRedeemStatus("error");
      setRedeemMessage(UIX.redeemErrGeneric);
    }
  };
  // Un seul niveau d'accès complet : "membre". La valeur "member" est celle
  // stockée en base ; "premiumPlus" reste accepté pour les comptes activés
  // avant le renommage, afin de ne pas leur couper l'accès.
  const isMember = effectiveTier === "member" || effectiveTier === "premiumPlus";
  // Plafond de crédits IA (affichage seulement — le serveur applique le sien
  // de façon indépendante, c'est lui qui fait autorité).
  const creditsCap = isMember ? 150 : null;
  // Rappelée par askClaude après un appel réussi (le serveur renvoie le nouveau
  // total) : garde le compteur à jour sans nécessiter un fetch séparé.
  const updateCreditsDisplay = (used) => { if (session) setCreditsUsed(used); };
  const [showTierModal, setShowTierModal] = useState(false);

  // ── SYNC CLOUD : ÉQUIPEMENT (membre connecté) ──────────────────────────
  // Politique simple : le cloud fait référence s'il existe déjà.
  // Migration : les entrées créées avant la synchronisation ont un id
  // numérique (Date.now()), incompatible avec la colonne uuid de Supabase —
  // on laisse Supabase générer ses propres ids lors de la première migration,
  // puis on adopte ces nouveaux ids localement pour rester synchronisé ensuite.
  useEffect(() => {
    if (!(session && isMember)) return;
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase.from("coffre_equipement").select("id, cat, titre, detail").eq("user_id", session.user.id);
      if (cancelled || error) return;
      if (data.length > 0) {
        setCoffreEquip(data);
      } else if (coffreEquip.length > 0) {
        const rows = coffreEquip.map(e => ({ user_id: session.user.id, cat: e.cat, titre: e.titre, detail: e.detail }));
        const { data: inserted, error: insErr } = await supabase.from("coffre_equipement").insert(rows).select();
        if (!cancelled && !insErr && inserted) { setCoffreEquip(inserted); }
      }
      try { localStorage.removeItem(EQUIP_KEY); } catch {}
    })();
    return () => { cancelled = true; };
  }, [userId, isMember]);

  // ── SYNC CLOUD : CARNET (membre connecté) ──────────────────────────────
  // Le contenu texte de chaque entrée est stocké dans une colonne JSONB
  // (data) — voir le schéma corrigé. Les PHOTOS sont gérées séparément :
  // seuls leurs CHEMINS de stockage (bucket carnet-photos) transitent par
  // cette colonne JSONB, jamais les données brutes elles-mêmes.
  useEffect(() => {
    if (!(session && isMember)) return;
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase.from("carnet_entries").select("id, data").eq("user_id", session.user.id);
      if (cancelled || error) return;
      if (data.length > 0) {
        const cloudEntries = await Promise.all(data.map(async (r) => {
          const { photos: photoPaths, ...rest } = r.data;
          const photos = (photoPaths && photoPaths.length > 0) ? await downloadCarnetPhotos(photoPaths) : [];
          return { ...rest, id: r.id, photos };
        }));
        if (cancelled) return;
        setCarnetEntries(cloudEntries);
      } else if (carnetEntries.length > 0) {
        const rows = carnetEntries.map(e => { const { photos, ...rest } = e; return { user_id: session.user.id, data: rest }; });
        const { data: inserted, error: insErr } = await supabase.from("carnet_entries").insert(rows).select();
        if (!cancelled && !insErr && inserted) {
          const withIds = inserted.map((r, i) => ({ ...carnetEntries[i], id: r.id }));
          setCarnetEntries(withIds);
          // Migration des photos en tâche de fond, une fois les vrais ids connus
          inserted.forEach((r, i) => {
            const photos = carnetEntries[i].photos;
            if (photos && photos.length > 0) {
              uploadCarnetPhotos(session.user.id, r.id, photos).then((paths) => {
                if (paths.length > 0) supabase.from("carnet_entries").update({ data: { ...rows[i].data, photos: paths } }).eq("id", r.id);
              });
            }
          });
        }
      }
      try { localStorage.removeItem(CARNET_KEY); } catch {}
    })();
    return () => { cancelled = true; };
  }, [userId, isMember]);

  // ── SYNC CLOUD : DOCUMENTS DU COFFRE (membre connecté) ─────────────────
  // Même politique que les autres : le cloud fait référence s'il a déjà des
  // documents, sinon les documents locaux existants sont migrés une fois.
  useEffect(() => {
    if (!(session && isMember)) return;
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase.from("coffre_documents").select("doc_type, file_path, file_name, mime_type").eq("user_id", session.user.id);
      if (cancelled || error) return;
      if (data.length > 0) {
        const cloudDocs = {};
        for (const row of data) {
          try {
            const { data: blob, error: dlErr } = await supabase.storage.from("coffre-documents").download(row.file_path);
            if (!dlErr && blob) {
              cloudDocs[row.doc_type] = { name: row.file_name, type: row.mime_type, data: await blobToDataURI(blob) };
            }
          } catch { /* document individuel en échec : ignoré, les autres continuent */ }
        }
        if (!cancelled && Object.keys(cloudDocs).length > 0) {
          setCoffreData(cloudDocs);
        }
      } else if (Object.keys(coffreData).length > 0) {
        for (const [docType, doc] of Object.entries(coffreData)) {
          uploadCoffreDocToStorage(session.user.id, docType, doc.data, doc.type, doc.name);
        }
      }
      try { localStorage.removeItem(COFFRE_KEY); } catch {}
    })();
    return () => { cancelled = true; };
  }, [userId, isMember]);
  ...
  // ── Quota membre partagé (Assistant IA + Conseil calibre + Conseil munition) ──
  const MEMBER_CAP = 150;
  const currentQuotaCap = MEMBER_CAP;
  const getQuotaMonthKey = () => `chassia_premium_${new Date().getFullYear()}_${new Date().getMonth()}`;
  const getQuotaMonthCount = () => {
    try { return parseInt(localStorage.getItem(getQuotaMonthKey()) || "0", 10); } catch { return 0; }
  };
  const consumeQuota = () => {
    const count = getQuotaMonthCount();
    if (count >= currentQuotaCap) return false;
    try { localStorage.setItem(getQuotaMonthKey(), String(count + 1)); } catch {}
    return true;
  };
  ...
  async function conseillerCalibre() {
    ...
    if (!consumeQuota()) { setConseilCalib(QUOTA_MSG); return; }
    ...
    try {
      const resp = await askClaude(prompt, T.system, [], isMember, UIX.aiTimeout, UIX.aiOffline, session?.access_token, updateCreditsDisplay);
      setConseilCalib(resp);
    } catch(e) {
      setConseilCalib(e?.quotaExceeded ? QUOTA_MSG : UIX.errorPrefix + (e?.message || String(e)));
    } finally {
      setLoadingC(false);
    }
  }
  ...
  async function conseillerMunition() {
    ...
    if (!consumeQuota()) { setConseilMun(QUOTA_MSG); return; }
    ...
    try {
      ...
      setConseilMun(e?.quotaExceeded ? QUOTA_MSG : UIX.errorPrefix + (e?.message || String(e)));
    } finally {
      setLoadingM(false);
    }
  }
  ...
  // ── Assistant IA state ──────────────────────────────────────────────────
  const MAX_FREE = 1;
  // Clé du jour recalculée à chaque appel : bascule correcte à minuit
  ...
  const canAsk = isMember || questionsUsed < MAX_FREE;

  async function poserQuestion() {
    if (!question.trim() || !canAsk || loadingQ) return;

    // Garde-fou technique : quota partagé de 150 crédits/mois (toutes fonctions IA confondues)
    if (isMember) {
      if (!consumeQuota()) {
    ...
    try {
      const resp = await askClaude(q, T.system, apiHistory, isMember, UIX.aiTimeout, UIX.aiOffline, session?.access_token, updateCreditsDisplay);
      setChatHistory([...newHistory, { role: "assistant", content: resp }]);
    } catch(e) {
      setChatHistory([...newHistory, { role: "assistant", content: e?.quotaExceeded ? QUOTA_MSG : UIX.errorPrefix + (e?.message || String(e)) }]);
    } finally {
      setLoadingQ(false);
    }
  }
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", padding: "20px 24px 0" }}>
          ...
          {tab === "cible" && (
            <div className="fade-in">
              ...
              <div style={card}>
                ...
                <div style={{ marginBottom: 14 }}>
                  <label style={fieldLabel}>{UIX.munOptional}</label>
          ...
          {tab === "calibre" && (
            <div className="fade-in">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  <div>
                    <label style={fieldLabel}>
                      {T.gibierLabel}
                      {isMember && <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color: COLORS.accent, marginLeft:8 }}>{UIX.upTo5}</span>}
                    </label>
                    <div>
                        <select onChange={e => {
                            const val = e.target.value;
                            if (!val) return;
                            const max = isMember ? 5 : 1;
                            setGibierCMulti(prev => prev.includes(val) ? prev : prev.length < max ? [...prev, val] : isMember ? prev : [val]);
                            e.target.value = "";
                          }}>
                        ...
                        <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color: COLORS.textDim, marginTop:3 }}>
                          {gibierCMulti.length}/{isMember ? 5 : 1} {UIX.selected}{lang==="fr" && gibierCMulti.length>1?"s":""}
                        </div>
                      </div>
                  </div>
                  <div>
              ...
              {gibierCMulti.length > 0 && (() => {
                ...
                return (
                  <div className="fade-in">
                    ...
                    {isMember ? (
                      <>
                      {creditsUsed != null && creditsCap != null && (
                        <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.textMuted, marginBottom:6, textAlign:"right" }}>
                          {creditsUsed}/{creditsCap} {UIX.creditsUnit}
                        </div>
                      )}
              ...
              {/* Invitation à se connecter — toujours visible */}
              {!isMember && gibierCMulti.length > 0 ? null : !isMember && (
            ...
            <div className="fade-in">
              ...
              {/* ── MODE CARABINE ── */}
                ...
                <div>
                      ...
                      <div>
                        ...
                        <div>
                            <select onChange={e => {
                                ...
                                if (!val) return;
                                const max = isMember ? 5 : 1;
                                setGibierMMulti(prev => prev.includes(val) ? prev : prev.length < max ? [...prev, val] : isMember ? prev : [val]);
                                setConseilMun(null);
                                e.target.value = "";
                              }}>
                              <option value="">{UIX.addGame}</option>
                            ...
                            <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color: COLORS.textDim, marginTop:3 }}>
                              {gibierMMulti.length}/{isMember ? 5 : 1} {UIX.selected}{lang==="fr" && gibierMMulti.length>1?"s":""}
                            </div>
                          </div>
                      </div>
                      <div>
                  ...
                  {/* Bouton IA carabine — membres uniquement */}
                  {isMember && (gibierMMulti.length > 0 || gibierM) && calibreM && (
                    <>
                    {creditsUsed != null && creditsCap != null && (
                      <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.textMuted, marginTop:8, marginBottom:-4, textAlign:"right" }}>
                        {creditsUsed}/{creditsCap} {UIX.creditsUnit}
                      </div>
                    )}
              ...
              {munMode === "fusil" && (
                <div>
                  <div style={card}>
                    ...
                    {/* Bouton IA — membres uniquement, visible après sélection */}
                    {isMember && (
                      <>
                      {creditsUsed != null && creditsCap != null && (
                        <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.textMuted, marginBottom:6, textAlign:"right" }}>
              ...
              {munMode === "comparateur" && (() => {
                ...
                const m3 = isMember ? list.find(m => m.label === compMun3) : null;
                const m4 = isMember ? list.find(m => m.label === compMun4) : null;
                ...
                return (
                  <div className="fade-in">
                    <p style={{ color: COLORS.textDim, fontSize:"0.95rem", marginBottom:16, fontStyle:"italic" }}>
                      {UIX.compareIntro} {isMember ? UIX.compareMax4 : isMember ? UIX.compareMax3 : UIX.compareFree}
                    </p>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
                      <div>
                          <label style={{...fieldLabel, color:COLORS.compare1}}>● {UIX.munition} 1</label>
                      ...
                      <div>
                          <label style={{...fieldLabel, color:COLORS.compare2}}>● {UIX.munition} 2</label>
                      ...
                      {isMember ? (
                        <div>
                            <label style={{...fieldLabel, color:COLORS.compare3}}>● {UIX.munition} 3 <span style={{fontSize:9, color:COLORS.accent}}>{TIER_MODAL_I18N[lang]?.memberName || "Membre"}</span></label>
                      ...
                      {isMember ? (
                        <div>
                            <label style={{...fieldLabel, color:COLORS.compare4}}>● {UIX.munition} 4 <span style={{fontSize:9, color:COLORS.accent}}>{TIER_MODAL_I18N[lang]?.memberName || "Membre"}</span></label>
          ...
          {tab === "assistant" && (
            <div className="fade-in">
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: 16 }}>
                  ...
                  {isMember && (
                    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                      <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize: 11, color: COLORS.accent }}>
                        🌿 {isMember ? "PREMIUM+" : "PREMIUM"} — {creditsUsed != null ? `${creditsUsed}/${currentQuotaCap}` : currentQuotaCap} {UIX.creditsLabel}
                      </div>
                      <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize: 10, color: COLORS.textDim, display:"flex", alignItems:"center", gap:5 }}>
                        <span style={{ color: COLORS.accent }}>⚡</span> {isMember ? UIX.opusAnalysis : UIX.sonnetAnalysis}
                      </div>
                      {!isMember && (
                ...
                {/* Visiteur : bouton d'accès · Membre : badge de statut */}
                {!isMember ? (
                  ...
                  <button onClick={openTierModal}
                    style={{
                      background: isMember ? COLORS.accent : COLORS.accentGlow,
                      border: `1px solid ${COLORS.accent}`,
                      borderRadius: 4, color: isMember ? "#0d0f0a" : COLORS.accent,
                      padding: "6px 14px", fontSize: 11,
                      fontFamily:"'IBM Plex Mono',monospace", cursor:"pointer",
                      transition: "all 0.2s", whiteSpace:"nowrap",
                    }}>
                    {isMember ? T.premiumPlusActive : T.premiumActive}
                  </button>
                )}
              </div>

              {/* Bloc premium si limite atteinte */}
              {!isMember && questionsUsed >= MAX_FREE && (
          ...
          {/* ── TAB 6 : À PROPOS ── */}
          {/* ── ONGLET CARNET DE CHASSE (membres) ── */}
          {tab === "carnet" && (
            <div className="fade-in">
              {!isMember ? (
                <div style={card}>
                ...
                return (
                  <div>
                    {isMember && <SyncBanner session={session} onSignIn={signInWithGoogle} onSignOut={signOutSupabase} texts={UIX} creditsUsed={creditsUsed} creditsCap={creditsCap} />}
                    {/* Header */}
                    ...
                    {/* Bannière sauvegarde — positive si synchronisé, avertissement sinon */}
                    {(session && isMember) ? (
                      <div style={{ background:COLORS.accentGlow, border:`1px solid ${COLORS.accent}40`, borderRadius:8, padding:"10px 14px", marginBottom:16, display:"flex", alignItems:"flex-start", gap:10 }}>
                        <span style={{ fontSize:14, flexShrink:0 }}>✓</span>
                        <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.accent, lineHeight:1.6 }}>
                          {CN.backupSynced}
                        </span>
                      </div>
                    ) : (
                      ...
                      <div style={card}>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                          ...
                          <div>
                            {FUSIL_CALIBRES.some(c => c.value === carnetDraft.calibre) ? (
                              ...
                              <>
                                <label style={lblStyle} htmlFor="cn-munition">{CN.fields.munition}</label>
                              ...
                              <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:4 }}>
                                {(carnetDraft.photos||[]).map((src, i) => (
                                  <div key={i} style={{ position:"relative", width:100, height:100 }}>
                                    <img src={src} alt="" onClick={() => openCarnetPhoto(src)} style={{ width:100, height:100, objectFit:"cover", borderRadius:6, border:`1px solid ${COLORS.border}`, cursor:"pointer" }}/>
                        ...
                        <div style={{ display:"flex", gap:10, marginTop:16 }}>
                          <button onClick={async () => {
                            ...
                            const isNewEntry = !carnetDraft.id;
                            const entry = { ...carnetDraft, id: carnetDraft.id || crypto.randomUUID() };
                            const updated = carnetDraft.id
                              ? carnetEntries.map(e => e.id === carnetDraft.id ? entry : e)
                              : [entry, ...carnetEntries];
                            const cloudEligible = session && isMember;
                            const { photos, ...rest } = entry; // photos gérées séparément (Storage)
                            if (cloudEligible) {
                              // On ATTEND l'enregistrement du texte avant de valider l'affichage :
                              // sans ça, une resynchronisation survenant entre-temps écrasait
                              // l'entrée qui n'était pas encore arrivée dans le cloud.
                              const { error } = isNewEntry
                                ? await supabase.from("carnet_entries").insert({ id: entry.id, user_id: session.user.id, data: rest })
                                : await supabase.from("carnet_entries").update({ data: rest }).eq("id", entry.id);
                              if (error) { setCarnetSaveError(CN.saveErr); return; }
                            } else if (!saveCarnetWithBackup(updated)) {
                              setCarnetSaveError(CN.saveErr); return; // rien de sauvé nulle part : reste sur le formulaire
                            }
                            setCarnetEntries(updated);
                            setCarnetSaison(carnetDraft.saison || getSaison());
                            setCarnetSaveError("");
                            setCarnetView("liste");
                            if (cloudEligible && photos && photos.length > 0) {
                              // Photos en tâche de fond : plus lentes, et leur retard n'efface
                              // jamais l'entrée elle-même (déjà enregistrée ci-dessus).
                              uploadCarnetPhotos(session.user.id, entry.id, photos).then(async (paths) => {
                                if (paths.length > 0) {
                                  await supabase.from("carnet_entries").update({ data: { ...rest, photos: paths } }).eq("id", entry.id);
                                }
                              });
                            }
                              ...
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  <div style={{ display:"flex", gap:4 }}>
                                    <button onClick={async () => {
                                      const u = carnetEntries.filter(e => e.id !== entry.id);
                                      const cloudEligible = session && isMember;
                                      if (cloudEligible) {
                                        // On ATTEND la confirmation Supabase avant de considérer la
                                        // suppression comme faite : sans ça, fermer l'app juste après
                                        // pouvait interrompre la requête, et l'entrée réapparaissait au
                                        // prochain chargement (le cloud, jamais réellement modifié,
                                        // faisait référence). C'est le vrai correctif du bug rapporté.
                                        await supabase.from("carnet_entries").delete().eq("id", entry.id);
                                        removeCarnetPhotoFolder(session.user.id, entry.id);
                                      } else {
                                        saveCarnetWithBackup(u);
                                      }
                                      setCarnetEntries(u);
                                      setConfirmDelete(null);
                                    }} style={{ background:COLORS.danger, border:"none", borderRadius:5, padding:"4px 10px", color:"white", fontSize:10, cursor:"pointer", fontWeight:700 }}>{CN.confirm}</button>
                              ...
                              {entry.photos && entry.photos.length > 0 && (
                                <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginTop:8 }}>
                                  {entry.photos.map((src, i) => (
                                    <img key={i} src={src} alt="" onClick={() => openCarnetPhoto(src)} style={{ width:56, height:56, objectFit:"cover", borderRadius:6, border:`1px solid ${COLORS.border}`, cursor:"pointer" }} />
                                  ))}
                                </div>
                              )}
                            </div>
          ...
          {tab === "coffre" && (
            <div className="fade-in">
              ...
              <p style={{ color:COLORS.textMuted, fontStyle:"italic", marginBottom:16, fontSize:"0.95rem" }}>
                {(session && isMember) ? CF.introSynced : CF.intro}
              </p>

              {/* Bannière stockage — positive si synchronisé, avertissement sinon */}
              {(session && isMember) ? (
                <div style={{ background:COLORS.accentGlow, border:`1px solid ${COLORS.accent}40`, borderRadius:8, padding:"10px 14px", marginBottom:20, display:"flex", alignItems:"flex-start", gap:10 }}>
                  <span style={{ fontSize:14, flexShrink:0 }}>✓</span>
                  <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.accent, lineHeight:1.6 }}>
                    {CF.bannerSynced}
                  </span>
                        ...
                        <div key={e.id} style={{ ...card, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                          ...
                          <button onClick={async () => {
                              if (confirmDeleteEquip === e.id) {
                                ...
                                const cloudEligible = session && isMember;
                                if (cloudEligible) {
                                  // Attendre la confirmation, sinon une resynchronisation
                                  // pouvait faire réapparaître l'entrée supprimée.
                                  const { error } = await supabase.from("coffre_equipement").delete().eq("id", e.id);
                                  if (error) { setEquipError(UIX.coffreErrStorage); setConfirmDeleteEquip(null); return; }
                                } else if (!saveEquip(updated)) {
                                  setEquipError(UIX.coffreErrStorage); setConfirmDeleteEquip(null); return;
                                }
                                setCoffreEquip(updated);
                        ...
                        <div style={{ ...card, padding:"12px 14px", display:"flex", flexDirection:"column", gap:8 }}>
                          ...
                          <div style={{ display:"flex", gap:8 }}>
                            <button onClick={async () => {
                                ...
                                const entry = { id:crypto.randomUUID(), cat, titre:equipForm.titre.trim(), detail:equipForm.detail.trim() };
                                const updated = [...coffreEquip, entry];
                                const cloudEligible = session && isMember;
                                if (cloudEligible) {
                                  // On ATTEND la confirmation avant d'afficher l'entrée : sans ça,
                                  // une resynchronisation déclenchée entre-temps (rafraîchissement
                                  // de session) écrasait l'affichage et l'entrée « disparaissait ».
                                  const { error } = await supabase.from("coffre_equipement")
                                    .insert({ id:entry.id, user_id:session.user.id, cat:entry.cat, titre:entry.titre, detail:entry.detail });
                                  if (error) { setEquipError(UIX.coffreErrStorage); return; }
                                } else if (!saveEquip(updated)) {
                                  setEquipError(UIX.coffreErrStorage); return;
                                }
                                setCoffreEquip(updated); setAddingEquipCat(null); setEquipForm({ titre:"", detail:"" }); setEquipError("");
                              }}
            ...
            <div className="fade-in">
              ...
              <div style={{ ...card, textAlign:"center" }}>
                <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color: COLORS.accent, marginBottom:6 }}>
                  {isMember ? (TIER_MODAL_I18N[lang]?.memberName || "Membre") : (TIER_MODAL_I18N[lang]?.visitorName || "Visiteur")}
                </div>
                {!isMember && (
                  <button onClick={openTierModal}
                    style={{ ...btnPrimary, marginTop:16, padding:"10px 32px" }}>
                    {isMember ? UIX.premiumPlusBtn : T.premiumBtn}
                  </button>
                )}
              </div>
      ...
      {showTierModal && (() => {
        ...
        const card = (name, features, active) => (
          <div style={{ border:`1px solid ${active ? COLORS.accent : COLORS.border}`, borderRadius:10, padding:"16px 18px", background: active ? COLORS.accentGlow : "transparent" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, marginBottom:10 }}>
              <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:14, fontWeight:700, color: active ? COLORS.accent : COLORS.white }}>{name}</span>
              {active && (
                <span style={{ background:COLORS.accent, color:COLORS.onAccent, borderRadius:5, padding:"3px 8px", fontFamily:"'IBM Plex Mono',monospace", fontSize:10, fontWeight:700, flexShrink:0 }}>{TM.currentBadge}</span>
              )}
            </div>
            <ul style={{ margin:0, paddingLeft:18, color:COLORS.textMuted, fontSize:"0.85rem", lineHeight:1.7 }}>
        ...
        return (
          <div onClick={() => setShowTierModal(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
            <div onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={TM.title} style={{ background:COLORS.bg, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:"24px 22px", maxWidth:480, width:"100%", maxHeight:"90vh", overflowY:"auto" }}>
              ...
              {redeemMessage && (
                <p style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, marginBottom:14, color: redeemStatus === "success" ? COLORS.accent : COLORS.danger }}>
                  {redeemStatus === "success" ? "✓" : "⚠️"} {redeemMessage}
                </p>
              )}
              ...
              {!session ? (
                <button onClick={signInWithGoogle}
                  style={{ display:"block", width:"100%", background:"transparent", border:`1px solid ${COLORS.border}`, borderRadius:6, color:COLORS.textMuted, padding:"9px 14px", fontFamily:"'IBM Plex Mono',monospace", fontSize:12, cursor:"pointer", marginBottom:14, textAlign:"center" }}>
                  {TM.connectBtn}
                </button>
              ) : (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap", marginBottom:14, fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:COLORS.textMuted }}>
                  <span style={{ minWidth:0, overflowWrap:"anywhere" }}>{UIX.syncConnectedPrefix} {session.user.email}</span>
                  <button onClick={signOutSupabase}
                    style={{ background:"transparent", border:"none", color:COLORS.textMuted, fontFamily:"'IBM Plex Mono',monospace", fontSize:11, cursor:"pointer", textDecoration:"underline", flexShrink:0, padding:0 }}>
                    {UIX.syncSignOut}
                  </button>
                </div>
              )}

              {/* Saisie du code d'accès — masquée si l'accès est déjà actif */}
              {!isMember && (
                <div style={{ display:"flex", gap:8, marginBottom:20 }}>
                  <input type="text" value={redeemCode} onChange={e => setRedeemCode(e.target.value)} placeholder={UIX.redeemPlaceholder}
                    style={{ flex:1, minWidth:0, background:COLORS.bgCard, border:`1px solid ${COLORS.border}`, borderRadius:6, padding:"9px 10px", color:COLORS.white, fontFamily:"'IBM Plex Mono',monospace", fontSize:13 }} />
                  <button onClick={() => activateCode(redeemCode, setRedeemCode)} disabled={redeemStatus === "loading" || !redeemCode.trim()}
                    style={{ background:COLORS.accent, color:COLORS.onAccent, border:"none", borderRadius:6, padding:"9px 16px", fontFamily:"'IBM Plex Mono',monospace", fontSize:12, fontWeight:700, cursor:"pointer", flexShrink:0, opacity:(redeemStatus === "loading" || !redeemCode.trim()) ? 0.6 : 1 }}>
                    {UIX.redeemBtn}
                  </button>
                </div>
              )}
              ...
              <div style={{ display:"grid", gap:12, marginBottom:18 }}>
                {card(TM.visitorName, TM.visitorFeatures, !isMember)}
                {card(TM.memberName, TM.memberFeatures, isMember)}
              </div>
