...
const UIX_I18N = {
  fr: {
    syncSignIn:"Se connecter avec Google", syncSignOut:"Se déconnecter", syncExplain:"Connecte-toi pour synchroniser tes données entre appareils.", syncConnectedPrefix:"Connecté :", tierUpdateError:"Échec de la mise à jour de ton abonnement. Réessaie dans un instant.", creditsUnit:"crédits", redeemTitle:"Tu as un code d'accès ?", redeemPlaceholder:"Ton code", redeemBtn:"Activer", redeemSuccess:"Code activé ! Ton palier est mis à jour.", redeemErrNotFound:"Code introuvable.", redeemErrUsed:"Ce code a déjà été utilisé.", redeemErrSession:"Connecte-toi avec Google pour activer ton code.", redeemErrGeneric:"Une erreur est survenue, réessaie dans un instant.",
  ...
  de: {
    syncSignIn:"Mit Google anmelden", syncSignOut:"Abmelden", syncExplain:"Melde dich an, um deine Daten zwischen Geräten zu synchronisieren.", syncConnectedPrefix:"Verbunden:", tierUpdateError:"Aktualisierung deines Abos fehlgeschlagen. Versuch es gleich noch einmal.", creditsUnit:"Credits", redeemTitle:"Hast du einen Zugangscode?", redeemPlaceholder:"Dein Code", redeemBtn:"Aktivieren", redeemSuccess:"Code aktiviert! Dein Level wurde aktualisiert.", redeemErrNotFound:"Code nicht gefunden.", redeemErrUsed:"Dieser Code wurde bereits verwendet.", redeemErrSession:"Melde dich mit Google an, um deinen Code zu aktivieren.", redeemErrGeneric:"Ein Fehler ist aufgetreten, versuch es gleich noch einmal.",
  ...
  en: {
    syncSignIn:"Sign in with Google", syncSignOut:"Sign out", syncExplain:"Sign in to sync your data across devices.", syncConnectedPrefix:"Connected:", tierUpdateError:"Failed to update your subscription. Try again shortly.", creditsUnit:"credits", redeemTitle:"Got an access code?", redeemPlaceholder:"Your code", redeemBtn:"Activate", redeemSuccess:"Code activated! Your tier has been updated.", redeemErrNotFound:"Code not found.", redeemErrUsed:"This code has already been used.", redeemErrSession:"Sign in with Google to activate your code.", redeemErrGeneric:"Something went wrong, try again shortly.",
...
export default function ChassIA() {
  ...
  const tier = "free"; // valeur pour un visiteur non connecté (voir chooseTier ci-dessous)
  // Une fois connecté, le vrai palier vient du profil Supabase (persiste au
  // rechargement, contrairement au bouton de test ci-dessus)
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      // Un choix de palier était-il en attente (mémorisé juste avant une connexion) ?
      let pending = null;
      try { pending = localStorage.getItem("chassia_pending_tier"); } catch {}
      if (pending) {
        try { localStorage.removeItem("chassia_pending_tier"); } catch {}
        await supabase.from("profiles").update({ tier: pending }).eq("id", session.user.id).select();
      }
  ...
  const chooseTier = async (chosen) => {
    const dbValue = chosen === "premiumPlus" ? "premium_plus" : chosen;
    setTierError("");
    if (session) {
      // .select() est indispensable : sans lui, une mise à jour qui ne touche
      // aucune ligne (profil absent) renvoie un succès silencieux.
      const { data, error } = await supabase.from("profiles").update({ tier: dbValue }).eq("id", session.user.id).select();
      if (error || !data || data.length === 0) { setTierError(UIX.tierUpdateError); return; }
      setProfileTier(chosen);
    } else {
      try { localStorage.setItem("chassia_pending_tier", dbValue); } catch {}
      signInWithGoogle();
    }
  ...
  const activateCode = async () => {
    ...
    if (!session) {
      ...
      // activé automatiquement au retour (même mécanisme que le choix de palier).
        ...
        return (
          <div onClick={() => setShowTierModal(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
            <div onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={TM.title} style={{ background:COLORS.bg, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:"24px 22px", maxWidth:480, width:"100%", maxHeight:"90vh", overflowY:"auto" }}>
              ...
              {tierError && (
                <p style={{ color:COLORS.danger, fontFamily:"'IBM Plex Mono',monospace", fontSize:11, marginBottom:14 }}>⚠️ {tierError}</p>
              )}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <div style={{ border:`1px solid ${effectiveTier === "premium" ? COLORS.accent : COLORS.border}`, borderRadius:10, padding:"16px 16px" }}>
                  ...
                  {effectiveTier !== "premium" && (
                    <button onClick={() => chooseTier("premium")}
                ...
                <div style={{ border:`1px solid ${COLORS.accent}`, borderRadius:10, padding:"16px 16px", background:COLORS.accentGlow }}>
                  ...
                  {!isPremiumPlus && (
                    <button onClick={() => chooseTier("premiumPlus")}
                      style={{ ...btnPrimary, width:"100%", marginTop:14, fontSize:12 }}>
                      {effectiveTier === "premium" ? TM.plusUpgradeCta : TM.plusCta}
                    </button>
                  )}
                </div>
              </div>
              ...
              {/* Activation d'un code d'accès temporaire */}
              <div style={{ borderTop:`1px solid ${COLORS.border}`, paddingTop:14, marginBottom:14 }}>
                <label style={{ display:"block", fontSize:11, color:COLORS.textMuted, marginBottom:6, fontFamily:"'IBM Plex Mono',monospace" }}>{UIX.redeemTitle}</label>
                <div style={{ display:"flex", gap:8 }}>
                  <input type="text" value={redeemCode} onChange={e => setRedeemCode(e.target.value)} placeholder={UIX.redeemPlaceholder}
                    style={{ flex:1, background:COLORS.bgCard, border:`1px solid ${COLORS.border}`, borderRadius:6, padding:"8px 10px", color:COLORS.white, fontFamily:"'IBM Plex Mono',monospace", fontSize:13 }} />
                  <button onClick={activateCode} disabled={redeemStatus === "loading" || !redeemCode.trim()}
                    style={{ background:COLORS.accent, color:COLORS.onAccent, border:"none", borderRadius:6, padding:"8px 16px", fontFamily:"'IBM Plex Mono',monospace", fontSize:12, fontWeight:700, cursor:"pointer", opacity: (redeemStatus === "loading" || !redeemCode.trim()) ? 0.6 : 1 }}>
                    {UIX.redeemBtn}
                  </button>
                </div>
                {redeemMessage && (
                  <p style={{ fontSize:11, marginTop:8, fontFamily:"'IBM Plex Mono',monospace", color: redeemStatus === "success" ? COLORS.accent : COLORS.danger }}>
