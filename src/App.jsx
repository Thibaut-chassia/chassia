...
export default function ChassIA() {
  ...
  const getSaison = (dateStr?: string) => {
  ...
  const [carnetForm, setCarnetForm] = useState(false);
          ...
          {tab === "carnet" && (
            <div className="fade-in">
              {!isPremium ? (
                ...
                // Stats
                const statEspeces = filtered.reduce((acc, e) => { acc[e.espece] = (acc[e.espece]||0)+1; return acc; }, {} as Record<string,number>);
                const statCalibres = filtered.reduce((acc, e) => { if(e.calibre) acc[e.calibre] = (acc[e.calibre]||0)+1; return acc; }, {} as Record<string,number>);
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                        ...
                        {Object.keys(statEspeces).length > 0 && (
                          <div style={card}>
                            ...
                            {(Object.entries(statEspeces) as [string,number][]).sort((a,b)=>b[1]-a[1]).map(([esp,n]) => (
                              <div key={esp} style={{ marginBottom:8 }}>
                                ...
                                <div style={{ height:4, background:COLORS.border, borderRadius:2 }}>
                                  <div style={{ height:4, background:COLORS.accent, borderRadius:2, width:`${((n as number)/filtered.length)*100}%`, transition:"width 0.4s" }}/>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        {Object.keys(statCalibres).length > 0 && (
                          <div style={card}>
                            ...
                            {(Object.entries(statCalibres) as [string,number][]).sort((a,b)=>b[1]-a[1]).map(([cal,n]) => (
                              <div key={cal} style={{ marginBottom:8 }}>
                                ...
                                <div style={{ height:4, background:COLORS.border, borderRadius:2 }}>
                                  <div style={{ height:4, background:"#1565c0", borderRadius:2, width:`${((n as number)/filtered.length)*100}%`, transition:"width 0.4s" }}/>
                                </div>
                              </div>
                            ))}
                          </div>
