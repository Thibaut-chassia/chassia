...
// Calibré sur tableaux officiels RUAG (GECO/RWS). Validé ≤0,3cm d'écart (50-300m, 2 réglages).
// v0 en m/s, bc (G1), dro = distance de réglage (m). Retourne d => point d'impact en cm (+ haut / - bas).
const makeTrajectory = (v0, bc, dro) => {
          ...
          {tab === "cible" && (
            <div className="fade-in">
              ...
              {calibre && customDist && customDRO && (() => {
                ...
                // Modèle balistique partagé (makeTrajectory) — calibré RUAG, validé ≤0,3cm
                const dropAt = makeTrajectory(v0, bc, dro);
          ...
          {tab === "munition" && (
            <div className="fade-in">
              ...
              {munMode === "comparateur" && (() => {
                ...
                const DISTS = [25,50,75,100,125,150,175,200,250,300];
                const f1 = m1 ? makeTrajectory(m1.v0, getBC(m1.label), compDRO) : null;
                const f2 = m2 ? makeTrajectory(m2.v0, getBC(m2.label), compDRO) : null;
                const f3 = m3 ? makeTrajectory(m3.v0, getBC(m3.label), compDRO) : null;
                // Échelle graphique
                const allVals = [];
