...
const TIER_MODAL_I18N = {
  fr: {
    ...
    plusName: "Premium+",
    plusFeatures: ["Tout ce qui est inclus dans Premium", "Comparateur jusqu'à 4 munitions (3 en Premium)", "Rapport de saison PDF (statistiques + détail des prélèvements)", "150 crédits IA / mois au lieu de 100", "Réponses sur Claude Opus — analyses plus approfondies"],
  ...
  nl: {
    ...
    plusName: "Premium+",
    plusFeatures: ["Alles wat in Premium is inbegrepen", "Vergelijker tot 4 munities (3 in Premium)", "PDF-seizoensrapport (statistieken + detail van de vangsten)", "150 AI-credits / maand in plaats van 100", "Antwoorden op Claude Opus — diepgaandere analyses"],
  ...
  de: {
    ...
    plusName: "Premium+",
    plusFeatures: ["Alles, was in Premium enthalten ist", "Vergleich mit bis zu 4 Munitionen (3 in Premium)", "Saisonbericht PDF (Statistiken + Detail der Abschüsse)", "150 KI-Credits / Monat statt 100", "Antworten auf Claude Opus — vertiefte Analysen"],
  ...
  en: {
    ...
    plusName: "Premium+",
    plusFeatures: ["Everything included in Premium", "Comparator with up to 4 ammunitions (3 in Premium)", "Season report PDF (statistics + detailed harvest log)", "150 AI credits / month instead of 100", "Answers on Claude Opus — more in-depth analysis"],
              ...
              {calibre && customDist && customDRO && (() => {
                ...
                const bc = (() => {
                  ...
                  if (munData) {
                    ...
                    // 1a-poids. Clé par poids (calibre|||gamme|||poids) — désambigue les gammes multi-poids
                    const wmC = munData.label.match(/(\d+[.,]?\d*)\s*g\s*(?:🌿)?\s*$/);
                    if (wmC) {
                      const wkC = calibre + "|||" + g + "|||" + parseFloat(wmC[1].replace(",", ".")).toFixed(1);
                      if (SPECIFIC_BC[wkC] != null) return SPECIFIC_BC[wkC];
                    }
                    // 1a. Clé exacte (gamme complète, ex "Precision Hunter ELD-X")
                    const exact = SPECIFIC_BC[calibre + "|||" + g];
                    if (exact) return exact;
                    // 1b. Sous-clés (ex "ELD-X" seul)
            ...
            <div className="fade-in">
              ...
              {munMode === "comparateur" && (() => {
                ...
                const getBC = (label) => {
                  ...
                  if (wm) {
                    const wk = `${compCalibre}|||${g}|||${parseFloat(wm[1].replace(",", ".")).toFixed(1)}`;
                  ...
                  for (const [gKey] of GAMME_BC) {
                    if (g.includes(gKey)) {
                      const spec = SPECIFIC_BC[`${compCalibre}|||${gKey}`];
              ...
              {!isPremium ? (
                ...
                return (
                  <div>
                    ...
                    {carnetView === "stats" && (
                      isPremiumPlus ? (
                        <button onClick={() => window.print()} style={{ ...btnPrimary, width:"100%", marginBottom:16, background:"transparent", border:`1px solid ${COLORS.accent}`, color:COLORS.accent, fontSize:12, display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                              ...
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  ...
                                  <div style={{ display:"flex", gap:6 }}>
                                    <button onClick={() => { setCarnetDraft(entry); setCarnetView("form"); }} aria-label={CN.formEdit} style={{ background:"transparent", border:`1px solid ${COLORS.border}`, borderRadius:5, padding:"4px 8px", color:COLORS.textMuted, fontSize:10, cursor:"pointer" }}>✏️</button>
                    ...
                    {/* Contenu imprimable du rapport de saison — caché à l'écran, visible uniquement à l'impression */}
                    {carnetView === "stats" && isPremiumPlus && (
                      <div className="chassia-print-carnet" style={{ display:"none" }}>
                        <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#000", margin:"0 0 4px" }}>ChassIA — {CN.reportTitle}</h1>
                        <p style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:"#333", margin:"0 0 16px" }}>
                          {CN.season} {carnetSaison} · {CN.reportGenerated} {new Date().toLocaleDateString(lang)}
                        </p>
                        <div style={{ marginBottom:16, fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:"#000", lineHeight:1.8 }}>
                          <div><strong>{CN.takesSeasonStat}</strong> : {filtered.length}</div>
                          {Object.keys(statEspeces).length > 0 && (
                            <div><strong>{CN.bySpecies}</strong> : {Object.entries(statEspeces).sort((a,b)=>b[1]-a[1]).map(([esp,n]) => `${spLabel(esp,lang)} (${n})`).join(" · ")}</div>
                          )}
                          {Object.keys(statCalibres).length > 0 && (
                            <div><strong>{CN.byCaliber}</strong> : {Object.entries(statCalibres).sort((a,b)=>b[1]-a[1]).map(([cal,n]) => `${cal} (${n})`).join(" · ")}</div>
                          )}
                          {filtered.some(e=>e.distance) && (
                            <div>
                              <strong>{CN.avgDist}</strong> : {Math.round(filtered.filter(e=>e.distance).reduce((a,e)=>a+parseFloat(e.distance),0)/filtered.filter(e=>e.distance).length)}m
                              {filtered.some(e=>e.poids) && <> · <strong>{CN.avgWeight}</strong> : {Math.round(filtered.filter(e=>e.poids).reduce((a,e)=>a+parseFloat(e.poids),0)/filtered.filter(e=>e.poids).length*10)/10}kg</>}
                            </div>
                          )}
                        </div>
                        {filtered.length > 0 ? (
                          <table style={{ width:"100%", borderCollapse:"collapse", fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:"#000" }}>
                            <thead>
                              <tr style={{ borderBottom:"1px solid #000" }}>
                                <th style={{textAlign:"left", padding:"3px 6px"}}>{CN.fields.date}</th>
                                <th style={{textAlign:"left", padding:"3px 6px"}}>{CN.fields.espece}</th>
                                <th style={{textAlign:"left", padding:"3px 6px"}}>{CN.fields.sexe}</th>
                                <th style={{textAlign:"left", padding:"3px 6px"}}>{CN.fields.calibre}</th>
                                <th style={{textAlign:"right", padding:"3px 6px"}}>{CN.fields.distance}</th>
                                <th style={{textAlign:"right", padding:"3px 6px"}}>{CN.fields.poids}</th>
                                <th style={{textAlign:"left", padding:"3px 6px"}}>{CN.fields.lieu}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {filtered.slice().sort((a,b)=>(a.date||"").localeCompare(b.date||"")).map(e => (
                                <tr key={e.id} style={{ borderBottom:"1px solid #ddd" }}>
                                  <td style={{padding:"3px 6px"}}>{e.date || "—"}</td>
                                  <td style={{padding:"3px 6px"}}>{spLabel(e.espece,lang)}</td>
                                  <td style={{padding:"3px 6px"}}>{CN.optSexe[e.sexe] || e.sexe || "—"}</td>
                                  <td style={{padding:"3px 6px"}}>{e.calibre || "—"}</td>
                                  <td style={{textAlign:"right", padding:"3px 6px"}}>{e.distance ? `${e.distance}m` : "—"}</td>
                                  <td style={{textAlign:"right", padding:"3px 6px"}}>{e.poids ? `${e.poids}kg` : "—"}</td>
                                  <td style={{padding:"3px 6px"}}>{e.lieu || "—"}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        ) : (
                          <p style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:11, color:"#666", fontStyle:"italic" }}>{CN.noData}</p>
                        )}
                      </div>
                    )}
                  </div>
