...
export default function ChassIA() {
  ...
  const toggleFavori = (cal, label) => {
    const key = `${cal}|||${label}`;
    const removing = favoris.includes(key);
    const updated = removing ? favoris.filter(k => k !== key) : [...favoris, key];
    const cloudEligible = session && isPremiumPlus;
    setFavoris(updated); // état en mémoire dans tous les cas (affichage immédiat)
    if (!cloudEligible) saveFavoris(updated); // localStorage seulement hors mode cloud-only
    if (cloudEligible) {
      if (removing) supabase.from("favoris").delete().eq("user_id", session.user.id).eq("munition_label", key);
      else supabase.from("favoris").insert({ user_id: session.user.id, munition_label: key });
    }
  };
  ...
  // ── SYNC CLOUD : FAVORIS (Premium+ connecté) ─────────────────────────────
  // Politique simple pour cette première version : à la connexion, si le
  // cloud a déjà des favoris, il devient la référence (remplace le local).
  // S'il est vide mais que des favoris existent localement, on les migre
  // une fois vers le cloud. Écriture en tâche de fond (pas de blocage UI ni
  // de message d'erreur par bascule individuelle — l'état local reste la
  // source immédiate pour l'affichage, quoi qu'il arrive côté réseau).
  // AUCUN stockage local pour Premium+ connecté : Supabase est la seule
  // source persistante. Le localStorage existant est nettoyé une fois le
  // cloud établi (pull ou migration), pour ne rien laisser traîner.
  useEffect(() => {
    if (!(session && isPremiumPlus)) return;
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase.from("favoris").select("munition_label").eq("user_id", session.user.id);
      if (cancelled || error) return;
      if (data.length > 0) {
        const cloudFavoris = data.map(r => r.munition_label);
        setFavoris(cloudFavoris);
      } else if (favoris.length > 0) {
        const rows = favoris.map(f => ({ user_id: session.user.id, munition_label: f }));
        supabase.from("favoris").insert(rows);
      }
      try { localStorage.removeItem(FAVORIS_KEY); } catch {}
    })();
    return () => { cancelled = true; };
  }, [session, isPremiumPlus]);

  // ── SYNC CLOUD : ÉQUIPEMENT (Premium+ connecté) ──────────────────────────
  // Même politique que les favoris (cloud fait référence s'il existe déjà).
