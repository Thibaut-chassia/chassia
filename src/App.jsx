                        ...
                        <div style={{ display:"flex", gap:10, marginTop:16 }}>
                          <button onClick={() => {
                            ...
                            const updated = carnetDraft.id
                              ...
                              : [entry, ...carnetEntries];
                            const cloudEligible = session && isPremiumPlus;
                            const localOk = cloudEligible ? true : saveCarnetWithBackup(updated);
                            if (!localOk) { setCarnetSaveError(CN.saveErr); return; } // rien de sauvé nulle part : reste sur le formulaire
                            setCarnetEntries(updated);
                            setCarnetSaison(carnetDraft.saison || getSaison());
                            setCarnetSaveError("");
                            setCarnetView("liste");
                            if (cloudEligible) {
                              const { photos, ...rest } = entry; // photos gérées séparément (Storage), voir ci-dessous
                              const savePromise = isNewEntry
                                ? supabase.from("carnet_entries").insert({ id: entry.id, user_id: session.user.id, data: rest })
                                : supabase.from("carnet_entries").update({ data: rest }).eq("id", entry.id);
                              savePromise.then(async () => {
                                if (photos && photos.length > 0) {
                                  const paths = await uploadCarnetPhotos(session.user.id, entry.id, photos);
                                  if (paths.length > 0) {
                        ...
                        <div key={e.id} style={{ ...card, padding:"12px 14px", display:"flex", alignItems:"center", gap:12 }}>
                          ...
                          <button onClick={() => {
                              if (confirmDeleteEquip === e.id) {
                                ...
                                const cloudEligible = session && isPremiumPlus;
                                const localOk = cloudEligible ? true : saveEquip(updated);
                                if (localOk) {
                                  setCoffreEquip(updated);
                                  if (cloudEligible) supabase.from("coffre_equipement").delete().eq("id", e.id);
                          ...
                          <div style={{ display:"flex", gap:8 }}>
                            <button onClick={() => {
                                ...
                                if (localOk) {
                                  ...
                                  if (cloudEligible) supabase.from("coffre_equipement").insert({ id:entry.id, user_id:session.user.id, cat:entry.cat, titre:entry.titre, detail:entry.detail });
                                }
                                else setEquipError(UIX.coffreErrStorage);
                              }}
