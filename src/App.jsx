                    ...
                    {isPremium ? (
                      ...
                      <>
                        ...
                        <button onClick={() => setIsPremium(true)}
                          ...
                          ★ Premium : Distance libre 10–300m
                        </button>
                      </>
                    )}
                  </div>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                          ...
                          {recSP && (
                            <div style={{ background: COLORS.bgInput, border:`1px solid ${COLORS.leadFree}40`, borderLeft:`3px solid ${COLORS.leadFree}`, borderRadius:7, padding:"12px 14px" }}>
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.leadFree, fontWeight:600 }}>🌿 Sans plomb 🌿</span>
                ...
                return (
                  <div className="fade-in">
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
                      <div>
                        <label style={{...fieldLabel, color:COLORS.compare1}}>● Munition 1</label>
                      ...
                      <div>
                        <label style={{...fieldLabel, color:COLORS.compare2}}>● Munition 2</label>
                      ...
                      {isPremium ? (
                        <div style={{ gridColumn:"1 / -1" }}>
                          <label style={{...fieldLabel, color:COLORS.compare3}}>● Munition 3 <span style={{fontSize:9, color:COLORS.accent}}>Premium</span></label>
              ...
              {!isPremium ? (
                <div style={card}>
                  <div style={{ textAlign:"center", padding:"20px 0" }}>
                    ...
                    <button onClick={() => setIsPremium(true)} style={{ background: COLORS.accent, color:COLORS.onAccent, border:"none", borderRadius:8, padding:"12px 28px", fontFamily:"'IBM Plex Mono',monospace", fontSize:13, fontWeight:700, cursor:"pointer" }}>★ Activer Premium</button>
                  </div>
                </div>
              ) : (() => {
                // Filtrer par saison
                      ...
                      return (
                        <div style={{ background:COLORS.warnBg, border:`1px solid ${COLORS.warnLine}40`, borderRadius:8, padding:"10px 14px", marginBottom:16, display:"flex", alignItems:"flex-start", gap:10 }}>
                          ...
                          <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.warnLine, lineHeight:1.6 }}>
                            Vos données sont sauvegardées sur cet appareil uniquement. Si vous changez d'appareil ou videz votre navigateur, elles seront perdues. Synchronisation cloud bientôt disponible.
                          </span>
                        </div>
                      );
                    })()}
                            ...
                            <div key={key}>
                              ...
                              {type === "select" ? (
                                <select value={carnetDraft[key]||""} onChange={e => setCarnetDraft(d=>({...d,[key]:e.target.value}))} style={fieldStyle}>
                                  <option value="">—</option>
                                  {opts.split(",").map(o => <option key={o} value={o}>{key === "espece" ? spLabel(o, lang) : o}</option>)}
                                </select>
                              ) : type === "calibre" ? (
                            ...
                            <div key={entry.id} style={{ ...card, padding:"12px 14px" }}>
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                                ...
                                {confirmDelete === entry.id ? (
                                  <div style={{ display:"flex", gap:4 }}>
                                    <button onClick={() => { const u=carnetEntries.filter(e=>e.id!==entry.id); setCarnetEntries(u); saveCarnetWithBackup(u); setConfirmDelete(null); }} style={{ background:COLORS.danger, border:"none", borderRadius:5, padding:"4px 10px", color:"white", fontSize:10, cursor:"pointer", fontWeight:700 }}>Confirmer</button>
          ...
          {tab === "coffre" && (
            <div className="fade-in">
              ...
              <div style={{ background:COLORS.warnBg, border:`1px solid ${COLORS.warnLine}40`, borderRadius:8, padding:"10px 14px", marginBottom:20, display:"flex", alignItems:"flex-start", gap:10 }}>
                <span style={{ fontSize:14, flexShrink:0 }}>⚠️</span>
                <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:COLORS.warnLine, lineHeight:1.6 }}>
                  Vos documents sont sauvegardés sur cet appareil uniquement. Si vous changez d'appareil ou videz votre navigateur, ils seront perdus. Synchronisation cloud bientôt disponible.
                </span>
              </div>
                          ...
                          <>
                            <button onClick={() => {
                              if (stored.type === "application/pdf") {
                                ...
                                const w = window.open();
                                w.document.write(`<html><head><title>${stored.name}</title><style>body{margin:0;background:#000;display:flex;flex-direction:column;align-items:center}button{position:fixed;top:12px;right:12px;background:#c8f135;color:#000;border:none;border-radius:8px;padding:8px 16px;font-size:14px;font-weight:700;cursor:pointer;z-index:9999}img{width:100%;height:auto;display:block;margin-top:0}</style></head><body><button onclick="window.close()">✕ Fermer</button><img src="${stored.data}"/></body></html>`);
                                w.document.close();
                              }
                            }}
